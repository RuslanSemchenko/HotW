...
ifneq ($(BUILD_CLIENT),0)
  ...
  ifneq ($(call bin_path, pkg-config),)
    ...
    SDL2_CFLAGS=$(shell pkg-config --silence-errors --cflags sdl2 | sed 's/-Dmain=SDL2_main//')
    SDL2_LIBS=$(shell pkg-config --silence-errors --libs sdl2)    
...
ifneq (,$(findstring "$(PLATFORM)", "linux" "gnu_kfreebsd" "kfreebsd-gnu"))
  ...
  CLIENT_LDFLAGS = -rdynamic -Wl,-rpath,'$ORIGIN'
  SHLIBEXT=so
  SHLIBCFLAGS=-fPIC -fvisibility=hidden
  SHLIBLDFLAGS=-shared $(LDFLAGS) -Wl,-rpath,'$ORIGIN'

  THREAD_LIBS=-lpthread
  LIBS=-ldl -lm

  CLIENT_LIBS=$(SDL2_LIBS)
  RENDERER_LIBS = $(SDL2_LIBS) -lGL
...
CLIENT_LIBS += -framework IOKit -framework SDL2 
RENDERER_LIBS += -framework OpenGL -framework SDL2
...
ifeq ($(USE_LOCAL_HEADERS),1)
  CLIENT_CFLAGS += -I$(SDL2HDIR)/include
  ifeq ($(ARCH),x86)
...
ifeq ($(PLATFORM),irix64)
  ...
  CLIENT_LIBS=-L/usr/X11/$(LIB) $(SDL2_LIBS) \
    -lX11 -lXext -lm
  RENDERER_LIBS = $(SDL2_LIBS) -lGL
...
ifeq ($(PLATFORM),sunos)
  ...
  CLIENT_LIBS +=$(SDL2_LIBS) -lX11 -lXext -liconv -lm
  RENDERER_LIBS = $(SDL2_LIBS) -lGL
...
ifeq ($(USE_CODEC_OPUS),1)
  ...
  ifeq ($(USE_INTERNAL_OPUS),1)
    ...
    CLIENT_CFLAGS += $(shell pkg-config --cflags opus opusfile)
...
debug:
	@$(MAKE) targets B=$(BD) CFLAGS="$(CFLAGS) $(BASE_CFLAGS) $(DEPEND_CFLAGS)" \
	  ...
	  OPTIMIZE="$(DEBUG_CFLAGS) -fsanitize=address -g -fno-omit-frame-pointer" \
	  OPTIMIZEVM="$(DEBUG_CFLAGS) -fsanitize=address -g -fno-omit-frame-pointer" \
	  ASMFLAGS="-m" \
...
ifneq ($(USE_RENDERER_DLOPEN),0)
$(B)/$(CLIENTBIN)$(FULLBINEXT): $(Q3OBJ) $(LIBSDL2MAIN)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) \
		-o $@ $(Q3OBJ) \
		$(LIBSDL2MAIN) $(CLIENT_LIBS) $(LIBS)

$(B)/renderer_opengl1_$(SHLIBNAME): $(Q3ROBJ) $(JPGOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CFLAGS) $(SHLIBLDFLAGS) -o $@ $(Q3ROBJ) $(JPGOBJ) \
		$(THREAD_LIBS) $(LIBSDL2MAIN) $(RENDERER_LIBS) $(LIBS)
...
$(B)/$(CLIENTBIN)$(FULLBINEXT): $(Q3OBJ) $(Q3ROBJ) $(JPGOBJ) $(LIBSDL2MAIN)
	$(echo_cmd) "LD $@"
 $(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) \
		-o $@ $(Q3OBJ) $(Q3ROBJ) $(JPGOBJ) \
		$(LIBSDL2MAIN) $(CLIENT_LIBS) $(RENDERER_LIBS) $(LIBS)

$(B)/$(CLIENTBIN)_opengl2$(FULLBINEXT): $(Q3OBJ) $(Q3R2OBJ) $(Q3R2STRINGOBJ) $(JPGOBJ) $(LIBSDL2MAIN)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) \
...
ifneq ($(strip $(LIBSDL2MAIN)),)
ifneq ($(strip $(LIBSDL2MAINSRC)),)
 $(LIBSDL2MAIN) : $(LIBSDL2MAINSRC)
	cp $< $@
	$(RANLIB) $@
endif
endif
