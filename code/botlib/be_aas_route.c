...
void AAS_UpdateAreaRoutingCache(aas_routingcache_t* areacache)
{
	...
	// ИСПРАВЛЕНИЕ: Увеличили размер массива с 128 до 1024, чтобы избежать переполнения
	unsigned short int t, startareatraveltimes[1024];
...
//	Com_Memset(aasworld.areaupdate, 0, aasworld.numareas * sizeof(aas_routingupdate_t));
	...
	while (updateliststart)
	{
		...
		for (i = 0, revlink = revreach->first; revlink; revlink = revlink->next, i++)
		{
			...
			// ИСПРАВЛЕНИЕ: Проверка границ массива startareatraveltimes
			if (i >= 1024) {
				// Если превысили размер, просто пропускаем, чтобы не упасть.
				// В идеале можно вывести warning.
				continue;
			}
