...
// Changes Globals:		-
//===========================================================================
// Исправлено C4028: Приведение типов параметров к хедеру
weightconfig_t* ReadWeightConfig(const char* filename)
{
	...
	while (PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "weight"))
		{
			...
			// Исправлено C4267 и заменено на безопасный strcpy_s
			nameLen = strlen(token.string) + 1;
			config->weights[config->numweights].name = (char*)GetClearedMemory((int)nameLen);
			strcpy_s(config->weights[config->numweights].name, nameLen, token.string);
...
// Changes Globals:		-
//===========================================================================
// Исправлено C4028: const char* name
...
static float FuzzyWeightUndecided_r(int* inventory, fuzzyseperator_t* fs)
{
	...
	if (inventory[fs->index] < fs->value)
	{
		...
		else return fs->minweight + (float)random() * (fs->maxweight - fs->minweight);
	} //end if
	else if (fs->next)
	{
		if (inventory[fs->index] < fs->next->value)
		{
			...
			else w1 = fs->minweight + (float)random() * (fs->maxweight - fs->minweight);
			//second weight
			if (fs->next->child) w2 = FuzzyWeight_r(inventory, fs->next->child);
			else w2 = fs->next->minweight + (float)random() * (fs->next->maxweight - fs->next->minweight);
			//the scale factor
			if (fs->next->value == MAX_INVENTORYVALUE) // is fs->next the default case?
				return w2;
			else
...
float FuzzyWeightUndecided(int* inventory, weightconfig_t* wc, int weightnum)
{
	...
	while (1)
	{
		...
		{
			if (s->child) s = s->child;
			else return s->minweight + (float)random() * (s->maxweight - s->minweight);
		} //end if
		else
		{
			if (s->next) s = s->next;
...
static void EvolveFuzzySeperator_r(fuzzyseperator_t* fs)
{
	...
	{
		// Исправлено C4244: Явное приведение double к float
		if (random() < 0.01) 
			fs->weight += (float)(crandom() * (fs->maxweight - fs->minweight));
		else 
			fs->weight += (float)(crandom() * (fs->maxweight - fs->minweight) * 0.5);
...
static void ScaleFuzzySeperatorBalanceRange_r(fuzzyseperator_t* fs, float scale)
{
	...
	{
		// Исправлено C4244
		float mid = (fs->minweight + fs->maxweight) * 0.5f;
		//get the weight between bounds
