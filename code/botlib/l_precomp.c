...
int PC_MergeTokens(token_t *t1, token_t *t2)
{
	...
	if (t1->type == TT_STRING && t2->type == TT_STRING)
	{
		...
		//Q_strcat without leading double quote
...
//		{ "__STDC__", BUILTIN_STDC },
	...
	for (i = 0; builtin[i].string; i++)
	{
		...
		define->name = (char *) GetMemory(strlen(builtin[i].string) + 1); // Allocate enough memory
		Q_strncpyz(define->name, builtin[i].string, strlen(builtin[i].string) + 1); // Use Q_strncpyz for secure copy
										...
										token_t **firsttoken, token_t **lasttoken)
{
	...
	switch(define->builtin)
	{
		...
		case BUILTIN_DATE:
		{
			...
			//free(curtime); // ctime returns a pointer to a static buffer
			token->type = TT_NAME;
			token->subtype = strlen(token->string);
			*firsttoken = token;
			*lasttoken = token;
...
int PC_Directive_include(source_t *source)
{
	...
	{
		StripDoubleQuotes(token.string); // NOTE: unsafe, but string is guaranteed to be quoted
...
int PC_Directive_define(source_t *source)
{
	...
	define = (define_t *) GetMemory(sizeof(define_t));
	Com_Memset(define, 0, sizeof(define_t));
	define->name = (char *) GetMemory(strlen(token.string) + 1); // Allocate enough memory
	Q_strncpyz(define->name, token.string, strlen(token.string) + 1); // Use Q_strncpyz for secure copy
	//add the define to the source
#if DEFINEHASHING
	PC_AddDefineToHash(define, source->definehash);
#else //DEFINEHASHING
