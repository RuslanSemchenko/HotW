...
int PS_ExpectTokenType(script_t *script, int type, int subtype, token_t *token)
{
	...
	if (token->type != type)
	{
		...
		else if (type == TT_LITERAL) Q_strncpyz(str, "literal", sizeof(str));
		else if (type == TT_NUMBER) Q_strncpyz(str, "number", sizeof(str));
		else if (type == TT_NAME) Q_strncpyz(str, "name", sizeof(str));
		else if (type == TT_PUNCTUATION) Q_strncpyz(str, "punctuation", sizeof(str));
	...
	if (token->type == TT_NUMBER)
	{
		if ((token->subtype & subtype) != subtype)
		{
			...
			else if (subtype & TT_HEX) Q_strncpyz(str, "hex", sizeof(str));
			else if (subtype & TT_OCTAL) Q_strncpyz(str, "octal", sizeof(str));
			else if (subtype & TT_BINARY) Q_strncpyz(str, "binary", sizeof(str));
...
int FileLength(FILE *fp)
{
	...
	if (end < 0) return 0; // Ensure non-negative length
