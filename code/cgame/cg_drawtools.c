...
#else
	CG_Text_Paint((float)x, (float)y, 0.3f, color, s, 0, 0, 0);
#endif
}
...
#else
	CG_Text_Paint((float)x, (float)y, 0.3f, color, s, 0, 0, 3);
#endif
}
...
static void UI_DrawBannerString2(int x, int y, const char* str, vec4_t color)
{
	...
	ay = (float)y * cgs.screenYScale;

	s = str;
	while (*s)
	{
		ch = (unsigned char)(*s & 127);
		if (ch == ' ') {
...
void UI_DrawBannerString(int x, int y, const char* str, int style, vec4_t color) {
	...
	if (style & UI_DROPSHADOW) {
		...
		drawcolor[3] = 0.5f;// patch colored loading screen: implement colored dropshadows
...
#endif
{
	...
	while (*s)
	{
		...
		if (Q_IsColorString(s)) {
			...
			newColor[3] = color[3];
			if (color[0] != 1.0f && color[1] != 1.0f && color[2] != 1.0f && color[3] == 0.5f) {
				newColor[0] *= 0.25f;
				newColor[1] *= 0.25f;
				newColor[2] *= 0.25f;
				newColor[3] = 0.85f;
			...
			trap_R_DrawStretchPic(ax, ay, aw, ah,
				fcol, frow, fcol + fwidth, frow + fheight,
				charset);
		}
		else {
			aw = 0.0f;
...
void UI_DrawProportionalString(int x, int y, const char* str, int style, vec4_t color) {
	...
	if (style & UI_PULSE) {
		...
		drawcolor[1] = color[1];
		drawcolor[2] = color[2];
		drawcolor[3] = 0.5f + 0.5f * (float)sin(cg.time / (double)PULSE_DIVISOR);
