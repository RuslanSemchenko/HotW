...
static void CG_Hit_Message( entityState_t *ent ){
	...
	Q_strcat( name, sizeof(name), S_COLOR_WHITE );
	...
	if(count){
		...
		for(i=0;i<count;i++){
			...
			if(ent->weapon)
				Q_strncpyz(part[i], hit_info[(int)ent->angles2[i]].forename, sizeof(part[i]));
			else
				Q_strncpyz(part[i], hit_info[(int)ent->angles2[i]].backname, sizeof(part[i]));
		}
...
static void CG_Obituary( entityState_t *ent ) {
	...
	Q_strcat( targetName, sizeof(targetName), S_COLOR_WHITE ); // S_COLOR_WHITE is "^7"
	...
	if ( !attackerInfo ) {
		...
		Q_strcat( attackerName, sizeof(attackerName), S_COLOR_WHITE ); // S_COLOR_WHITE is "^7"
	...
	if (attacker != ENTITYNUM_WORLD){
		...
		for(i = MAX_DEATH_MESSAGES-1; i; i--){
			if(cg.messagetime[i-1]){
				Q_strncpyz(cg.attacker[i], cg.attacker[i-1], sizeof(cg.attacker[i]));
				Q_strncpyz(cg.target[i], cg.target[i-1], sizeof(cg.target[i]));
				cg.messagetime[i] = cg.messagetime[i-1];
				cg.mod[i] = cg.mod[i-1];
			}
		}
		//the new field
		Q_strncpyz(cg.attacker[0], attackerName, sizeof(cg.attacker[0]));
		Q_strncpyz(cg.target[0], targetName, sizeof(cg.target[0]));
