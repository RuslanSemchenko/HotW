...
void CG_BloodTrail(localEntity_t * le) {
	...
	if (t <= cg.time && t > cg.oldTime) {
		...
		if (!whiskey) {
			...
			blood->refEntity.radius = 1.0f + (float)(rand() % 10) / 6.0f;
...
void CG_FragmentBounceMark(localEntity_t * le, trace_t * trace) {
	...
	if (le->leMarkType == LEMT_BLOOD) {
		...
		radius = 16 + (rand() & 31);
		CG_ImpactMark(cgs.media.bloodMarkShader, trace->endpos, trace->plane.normal, (float)(random() * 360),
#ifndef HotW
			1, 1, 1, 1, qtrue, radius, qfalse);
#else
			1.0f, 1.0f, 1.0f, 1.0f, qtrue, (float)radius, qfalse, -1 );
#endif
	}
	else if (le->leMarkType == LEMT_BURN) {

		radius = 8 + (rand() & 15);
		CG_ImpactMark(cgs.media.burnMarkShader, trace->endpos, trace->plane.normal, (float)(random() * 360),
#ifndef HotW
			1, 1, 1, 1, qtrue, radius, qfalse);
#else
			1.0f, 1.0f, 1.0f, 1.0f, qtrue, (float)radius, qfalse, -1 );
...
void CG_ReflectVelocity(localEntity_t * le, trace_t * trace) {
	...
	VectorMA(velocity, -2.0f * dot, trace->plane.normal, le->pos.trDelta);
	...
	if (trace->allsolid ||
		(trace->plane.normal[2] > 0 &&
			(le->pos.trDelta[2] < 40 || le->pos.trDelta[2] < (float)-cg.frametime * le->pos.trDelta[2]))) {
...
void CG_CreateFire(vec3_t origin, vec3_t normal) {
	...
	le->refEntity.rotation = (float)(rand() % 360);
	le->endTime = cg.time + WHISKEY_BURNTIME + rand() % 5000;
	le->refEntity.radius = (float)(25 + rand() % 16);

	// calculate lighting
	CG_LightForPoint(le->refEntity.origin, normal, light);

	factor = ((light[0] + light[1] + light[2]) / 3.0f) / 255.0f;
	factor *= 1.3f;
	...
	CG_ImpactMark(cgs.media.burnMarkShader, origin, normal,
		(float)(random() * 360), red, green, blue, 1.0f, qtrue, (float)((rand() % 8) + 25), qfalse,
		WHISKEY_SICKERTIME);
}
#endif
...
void CG_AddFragment(localEntity_t * le) {
	...
	if (le->pos.trType == TR_STATIONARY) {
		...
		if (t < SINK_TIME) {
			...
			le->refEntity.origin[2] -= 16.0f * (1.0f - (float)t / SINK_TIME);
	...
	if ((le->leFlags & LEF_WHISKEY) && (le->leFlags & LEF_FIRE) &&
		...
		fire.renderfx = 0;
		fire.shaderTime = (float)le->startTime / 1000.0f;
		...
		if (le->startTime + 250 > cg.time) {
			...
			fire.radius = (float)sqrt(t / 250.0f) * le->refEntity.radius;
			fire.origin[2] -= (float)sqrt((250.0f - t) / 250.0f) * 5.0f;
			...
			fire.radius = (float)sqrt(t / 5000.0f) * le->refEntity.radius;
			fire.origin[2] -= (float)sqrt((5000.0f - t) / 5000.0f) * 5.0f;
		}
	...
	if (le->refEntity.reType == RT_SPRITE && (le->leFlags & LEF_PARTICLE)) {
		...
		if (Distance(le->refEntity.origin, cg.refdef.vieworg) > 500.0f)
			visible = qfalse;
		...
		for (i = 0; i < 3; i++) {
			float temp = light[i] / 255.0f;
			temp = (float)sqrt(temp);
			temp *= 255.0f;

			color[i] *= temp * 0.87f + 255.0f * 0.13f;
		}
		color[3] *= 255.0f;
	...
	if (le->pos.trType == TR_STATIONARY && le->refEntity.reType != RT_SPRITE) {
		...
		if ((le->leFlags & LEF_MARK) && le->refEntity.radius != -1.0f &&
	...
	if (trace.fraction == 1.0f || (le->leFlags & LEF_SPARKS)) {
#else
	if (trace.fraction == 1.0f) {
		...
		if (le->leFlags & LEF_FIRE) {
			...
			if (le->lightColor[0] == 1) {
				...
				origin[2] += 20.0f;
				trap_R_AddLightToScene(origin, 50.0f,
					cg_weapons[WP_MOLOTOV].missileDlightColor[0],
					cg_weapons[WP_MOLOTOV].missileDlightColor[1],
					cg_weapons[WP_MOLOTOV].missileDlightColor[2]);
			}
	...
	if (le->leFlags & LEF_BLOOD) {
		...
		if ((le->leFlags & LEF_WHISKEY) && (le->leFlags & LEF_FIRE)) {
			...
			CG_ImpactMark(shader, trace.endpos, trace.plane.normal,
				(float)(random() * 360), red, green, blue, 1.0f, qtrue, radius, qfalse,
				time);
			CG_FreeLocalEntity(le);
		}
		return;
	...
	if (((le->leFlags & LEF_BREAKS) || (le->leFlags & LEF_BREAKS_DEF))
		&& trace.fraction < 1.0f) {
		le->angles.trBase[0] = 0.0f;
		le->angles.trBase[2] = 90.0f;
...
void CG_AddSmoke(localEntity_t * le) {
	...
	float boost = SPEED * (float)le->boost / 5.0f;
	float wind = SPEED * (float)le->wind / 5.0f;
	...
	re.rotation += (float)fade_in / 20.0f;

	Vector4Copy(le->color, re.shaderRGBA);
	temp = (float)le->color[3];

	if ((float)fade_in <= FADE_IN) {
		temp = (float)le->color[3] * ((float)fade_in / FADE_IN);
		re.radius *= ((float)fade_in / FADE_IN);
	}
	else if ((float)fade_out <= FADE_OUT) {
		temp = (float)le->color[3] * ((float)fade_out / FADE_OUT);
		re.radius *= 1.0f + ((FADE_OUT - (float)fade_out) / 750.0f);
	}

	re.shaderRGBA[3] = (byte)temp;

	re.reType = RT_SPRITE;

	// move up slowly
	re.origin[2] += (float)fade_in * boost;

	// if there's wind
	if ((le->vector[0] || le->vector[1] || le->vector[2]) &&
		le->wind) {
		// the first seconds let the wind fade in
		if ((float)fade_in < FADE_IN * 2.0f) {
			windfactor = (float)fade_in / (FADE_IN * 2.0f);
		}
		VectorMA(re.origin, (float)fade_in * wind * windfactor, le->vector, re.origin);
	}
...
static void CG_AddMoveScaleFade(localEntity_t * le) {
	...
	if (le->fadeInTime > le->startTime && cg.time < le->fadeInTime) {
		// fade / grow time
		c = 1.0f - (float)(le->fadeInTime - cg.time) / (float)(le->fadeInTime - le->startTime);
	}
	else {
		// fade / grow time
		c = (float)(le->endTime - cg.time) * le->lifeRate;
	...
	re->shaderRGBA[3] = (byte)(255.0f * c * le->color[3]);

	if (!(le->leFlags & LEF_PUFF_DONT_SCALE)) {
		re->radius = le->radius * (1.0f - c) + 8.0f;
	}
...
static void CG_AddFallScaleFade(localEntity_t * le) {
	...
	re->radius = le->radius * (1.0f - c) + 16.0f;
...
static void CG_AddExplosion(localEntity_t * ex) {
	...
	if (ex->light) {
		...
		light = (float)(cg.time - ex->startTime) / (float)(ex->endTime - ex->startTime);
		if (light < 0.5f) {
			light = 1.0f;
		}
		else {
...
static void CG_AddSpriteExplosion(localEntity_t * le) {
	...
	c = (float)(le->endTime - cg.time) / (float)(le->endTime - le->startTime);
	if (c > 1.0f) {
	...
	re.shaderRGBA[3] = (byte)(255.0f * c * 0.33f);
	...
	if (le->light) {
		...
		light = (float)(cg.time - le->startTime) / (float)(le->endTime - le->startTime);
		if (light < 0.5f) {
			light = 1.0f;
		}
		else {
