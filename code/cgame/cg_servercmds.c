...
void CG_ShaderStateChanged(void) {
	...
	while (o && *o) {
		n = strstr(o, "=");
		if (n && *n) {
			size_t lenOrig = (size_t)(n - o);
			if (lenOrig >= sizeof(originalShader)) lenOrig = sizeof(originalShader) - 1;
			strncpy_s(originalShader, sizeof(originalShader), o, lenOrig);
			originalShader[lenOrig] = '\0';

			n++;
			t = strstr(n, ":");
			if (t && *t) {
				size_t lenNew = (size_t)(t - n);
				if (lenNew >= sizeof(newShader)) lenNew = sizeof(newShader) - 1;
				strncpy_s(newShader, sizeof(newShader), n, lenNew);
				newShader[lenNew] = '\0';
			}
			else {
				break;
			...
			o = strstr(t, "@");
			if (o) {
				size_t lenTime = (size_t)(o - t);
				if (lenTime >= sizeof(timeOffset)) lenTime = sizeof(timeOffset) - 1;
				strncpy_s(timeOffset, sizeof(timeOffset), t, lenTime);
				timeOffset[lenTime] = '\0';
