...
static void CG_MachineGunEjectBrass(centity_t * cent) {
	...
	le->endTime = le->startTime + cg_brassTime.integer + (int)((cg_brassTime.integer / 4) * random());
...
static void CG_ShotgunEjectBrass(centity_t * cent) {
	...
	for (i = 0; i < 2; i++) {
		...
		le->leType = LE_FRAGMENT;
		le->startTime = cg.time;
		le->endTime = le->startTime + cg_brassTime.integer * 3 + (int)(cg_brassTime.integer * random());
...
#define SPACING  5
	...
	for (i = 0; i < 36; i++)
	{
		RotatePointAroundVector(axis[i], vec, temp, i * 10.0f);//banshee 2.4 was 10
	}
...
void CG_RegisterWeapon(int weaponNum) {
...
#else
	Q_strncpyz(path, bg_weaponlist[weaponNum].path, sizeof(path));
#endif
	COM_StripExtension(path, path, sizeof(path));
#ifndef HotW
	Q_strcat(path, sizeof(path), "_flash.md3");
#else
	Q_strcat(path, sizeof(path), "flash.md3");
#endif
	...
	if (bg_weaponlist[weaponNum].v_model) {
		Q_strncpyz(path, bg_weaponlist[weaponNum].v_model, sizeof(path));
		...
		if (bg_weaponlist[weaponNum].wp_sort == WPS_PISTOL) {
			COM_StripExtension(path, path, sizeof(path));
			Q_strcat(path, sizeof(path), "_l.md3");
...
#else
	case WP_KNIFE:
		weaponInfo->missileModel = trap_R_RegisterModel("models/weapons2/knife/knife");
		break;
...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles) {
	...
	// idle drift
	scale = cg.xyspeed + 40;
	fracsin = (float)sin(cg.time * 0.001);
	...
	angles[PITCH] += scale * fracsin * 0.01f + value;
...
void CG_AddPlayerWeapon(refEntity_t * parent, playerState_t * ps, centity_t * cent, int team) {
	...
	if (ps) {
		...
		if (ps->weapon2) {
			...
			CG_RunLerpFrame(ci, &cent->pe.weapon2, cent->pe.weapon2.weaponAnim, 1.0f, weapon2Num);
		}

		// 1st weapon
		CG_InitWeaponAnim(cent, weaponNum, cg.weaponAnim, &cent->pe.weapon, qfalse);
		CG_RunLerpFrame(ci, &cent->pe.weapon, cent->pe.weapon.weaponAnim, 1.0f, weaponNum);
	}
#endif
	...
	if (!ps) {
		...
		if (weaponNum == WP_DYNAMITE) {
			VectorScale(gun.axis[0], 0.5f, gun.axis[0]);
			VectorScale(gun.axis[1], 0.5f, gun.axis[1]);
			VectorScale(gun.axis[2], 0.5f, gun.axis[2]);
		}
		else if (weaponNum == WP_MOLOTOV) {
			VectorScale(gun.axis[0], 0.65f, gun.axis[0]);
			VectorScale(gun.axis[1], 0.65f, gun.axis[1]);
			VectorScale(gun.axis[2], 0.65f, gun.axis[2]);
		}
	}
	else {
		vec3_t forward;
...
void CG_NextWeapon_f(void) {
...
#ifndef HotW
	// (Код для оригинального Q3 пропущен, он не используется в HotW)
#else
	...
	// Защита от выхода за границы массива для startnum
	if (startnum < 0 || startnum >= WP_NUM_WEAPONS) {
		startnum = 0;
	}
	...
	for (j = 0; j < WPS_NUM_ITEMS; j++) {
		...
		// Player has two equal pistols... (логика пистолетов)
		if ((cg.snap->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL)
			&& num == WPS_PISTOL && j > 0)
		...
		for (i = 1, weapon = 0; i <= WP_NUM_WEAPONS; i++) {
			...
			if (num == WPS_PISTOL && weapon + 1 == WP_NUM_WEAPONS &&
				...
				// --- ИСПРАВЛЕНИЕ МАТЕМАТИКИ ---
				// Используем правильный циклический сдвиг
				while (weapon >= WP_NUM_WEAPONS)
					weapon -= WP_NUM_WEAPONS;
				// ------------------------------
			}

			// Финальная защита от дурака (ASan)
			if (weapon < 0 || weapon >= WP_NUM_WEAPONS) continue;
...
void CG_PrevWeapon_f(void) {
	...
	// Защита индекса
	if (startnum < 0 || startnum >= WP_NUM_WEAPONS) startnum = 0;

	for (j = 0; j < WPS_NUM_ITEMS; j++) {
		// Логика "Предыдущего" (Prev) оружия часто ищет в обратном порядке слотов
		// Но в SG оригинальный код использует сложение для слотов и вычитание для оружия.
		// Оставляем логику слотов как есть (если она была j++), но исправляем weapons.
		...
		if (startnum == WP_NONE) {
			...
			// ИСПРАВЛЕНИЕ: Защита
			if (startnum >= 0 && startnum < WP_NUM_WEAPONS)
				num = bg_weaponlist[startnum].wp_sort - j; // Здесь часто используется минус для Prev
			else
				num = WPS_PISTOL;

			// Если ушли в минус по слотам
			if (num < 0) num += WPS_NUM_ITEMS;
		}

		// Нормализация num (на всякий случай)
		while (num < 0) num += WPS_NUM_ITEMS;
		while (num >= WPS_NUM_ITEMS) num -= WPS_NUM_ITEMS;
		...
		for (i = 1, weapon = 0; i <= WP_NUM_WEAPONS; i++) {
			...
			if (num == WPS_PISTOL && weapon + 1 == WP_NUM_WEAPONS &&
				...
				if (cg.weaponSelect == WP_AKIMBO)
					weapon = WP_PEACEMAKER - i; // Минус для Prev
				else
					weapon = cg.weaponSelect - i; // Минус для Prev

				if (weapon == cg.weaponSelect)
					continue;

				// --- ИСПРАВЛЕНИЕ: Заворот при отрицательных значениях ---
				while (weapon < 0)
					weapon += WP_NUM_WEAPONS;
				while (weapon >= WP_NUM_WEAPONS) // На случай странной математики
					weapon -= WP_NUM_WEAPONS;
				// -------------------------------------------------------
				...
				weapon = cg.markedweapon - i; // Минус для Prev
			...
			// ИСПРАВЛЕНИЕ: Финальная защита (ASan)
			if (weapon < 0 || weapon >= WP_NUM_WEAPONS) continue;
...
void CG_BottleBreak(vec3_t org, vec3_t bottledir, qboolean fire, vec3_t dirs[ALC_COUNT]) {
	...
	for (i = 0; i < 4; i++) {
		...
		VectorSet(dir, ((rand() % 10) / 5.0f) - 1.0f, ((rand() % 10) / 5.0f) - 1.0f, 1);
	...
	for (i = 0; i < 8; i++) {
		...
		le->refEntity.radius = 1.0f + (rand() % 10) / 6.0f;
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir,
	...
	case WP_DYNAMITE:
		...
		mark = cgs.media.burnMarkShader;
		duration = 1000;
		radius_mrk = radius_exp = 64 * 2.5f + rand() % 30; //old 64
		light = 150;//300
		isSprite = qtrue;
		break;
	case WP_MOLOTOV:
	...
	if (mark) {
		...
		if (shaderNum > -1) {
			vec4_t	explight, color;
			int		j;

			Vector4Copy(shaderInfo[shaderNum].color, color);
			CG_LightForPoint(origin, dir, explight);

			for (j = 0; j < 3; j++) {
				float temp = explight[j] / 255.0f;
				temp = sqrtf(temp);
...
static void CG_ShotgunPellet(vec3_t start, vec3_t origin2, vec3_t end, int skipNum, int playerhit,
	...
	int sourceContentType, destContentType;
	int	shaderNum = 0;
	float		damage = (float)bg_weaponlist[weapon].damage;
	qboolean	shootthru;
	vec3_t		tr_start;
	int shootcount = 0;
...
cg_shotgunfire:
	...
	if (cg_entities[tr.entityNum].currentState.eType == ET_PLAYER) {
		...
		if (tr.entityNum == cg.snap->ps.clientNum) {
			//"shake" the view
			cg.landAngleChange = (rand() % 30) - 15.0f;
			cg.landTime = cg.time;
			cg.landChange = 0;
		}
	}
...
static void CG_ShotgunPattern(vec3_t origin, vec3_t origin2, int otherEntNum, entityState_t * es) {
	...
	if (cgs.newShotgunPattern) {
		...
		max_spread_circle = (float)count / (float)pellet_per_circle;
		if (max_spread_circle < 1.0f)  max_spread_circle = 1.0f;
		angle_shift = Q_random(&seed) * (float)M_PI * 2.0f;
		...
		for (i = -extra_center_pellet; i < count - extra_center_pellet; i++) {
			...
			if (extra_center_pellet > 0) {
				if (i < 0) {
					...
					current_spread_circle = extra_circle + (float)i / (float)pellet_per_circle;
					current_pellet_per_circle = pellet_per_circle;
				}
				current_spread_cell = i - (int)(current_spread_circle * (float)current_pellet_per_circle);
			}
			else {
				current_spread_circle = (float)i / (float)pellet_per_circle;
				current_pellet_per_circle = pellet_per_circle;
				current_spread_cell = i - (int)(current_spread_circle * (float)current_pellet_per_circle);
			}
			current_angle_shift = angle_shift + current_spread_circle * (float)M_PI / (float)current_pellet_per_circle;

			spread_dist = (current_spread_circle + Q_random(&seed)) / max_spread_circle * (float)bg_weaponlist[es->weapon].spread * 16.0f;
			...
			spread_angle = current_angle_shift + ((float)current_spread_cell + Q_random(&seed)) * (float)M_PI * 2.0f / (float)current_pellet_per_circle;

			r = (float)sin(spread_angle) * spread_dist;
			u = (float)cos(spread_angle) * spread_dist;
		...
		// generate the "random" spread pattern
		for (i = 0; i < count; i++) {
			r = Q_crandom(&seed) * (float)bg_weaponlist[es->weapon].spread * 16.0f;
			u = Q_crandom(&seed) * (float)bg_weaponlist[es->weapon].spread * 16.0f;
			VectorMA(origin, 8192 * 16, forward, end);
			VectorMA(end, r, right, end);
			VectorMA(end, u, up, end);
