	...
	if( !badWords )
	{
		...
		if( strlen( buf ) > 0 )
		{
			...
			while( ( at = strchr( start, ',' ) ) )
			{
				...
				*at = '\0'; // Temporarily terminate the string to copy just the word
				Q_strncpyz( blockThis, start, sizeof(blockThis) );
				*at = ','; // Restore the comma
				badWords[l] = strdup( blockThis );
				start = at + 1;
				++l;
			}

			badWords[l] = strdup( start );
			++l;
