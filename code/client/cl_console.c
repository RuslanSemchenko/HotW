...
void Con_MessageMode3_f(void) {
	// FIX C4244: Explicit cast from intptr_t to int
	chat_playerNum = (int)VM_Call(cgvm, CG_CROSSHAIR_PLAYER);
...
void Con_MessageMode4_f(void) {
	// FIX C4244: Explicit cast from intptr_t to int
	chat_playerNum = (int)VM_Call(cgvm, CG_LAST_ATTACKER);
	if (chat_playerNum < 0 || chat_playerNum >= MAX_CLIENTS) {
		chat_playerNum = -1;
		return;
	}
...
void Con_DrawInput(void) {
	...
	// FIX C4244: Explicit cast float to int for coordinates
...
void Con_DrawSolidConsole(float frac) {
	...
	if (y < 1) {
...
#ifndef HotW
		// FIX C4244: Explicit cast int to float for SCR_DrawPic
		SCR_DrawPic(0.0f, 0.0f, (float)SCREEN_WIDTH, (float)y, cls.consoleShader);
#else
		...
		if (cl_consoleType->integer) {
			// FIX C4244: Explicit cast int to float for SCR_DrawPic
			SCR_DrawPic(0.0f, 0.0f, (float)SCREEN_WIDTH, (float)y, cls.whiteShader);
		}
		else {
			// FIX C4244: Explicit cast int to float for SCR_DrawPic
			SCR_DrawPic(0.0f, 0.0f, (float)SCREEN_WIDTH, (float)y, cls.consoleShader);
...
#endif
	// FIX C4244: Explicit cast int to float for SCR_FillRect coordinates
	SCR_FillRect(0.0f, (float)y, (float)SCREEN_WIDTH, 2.0f, color);
