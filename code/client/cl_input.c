...
void IN_CenterView(void) {
	// FIX C4244: Explicit cast from double to vec_t
	cl.viewangles[PITCH] = (vec_t)-SHORT2ANGLE(cl.snap.ps.delta_angles[PITCH]);
}
...
void CL_KeyMove(usercmd_t* cmd) {
	...
	up = 0;
	if (in_strafe.active) {
		// FIX C4244: Explicit cast to int
		side += (int)(movespeed * CL_KeyState(&in_right));
		side -= (int)(movespeed * CL_KeyState(&in_left));
	}

	// FIX C4244: Explicit cast to int
	side += (int)(movespeed * CL_KeyState(&in_moveright));
	side -= (int)(movespeed * CL_KeyState(&in_moveleft));
...
void CL_MouseEvent(int dx, int dy, int time) {
	// FIX C4100: Unreferenced parameter
	(void)time;
...
void CL_MouseMove(usercmd_t* cmd)
{
	...
	if (m_filter->integer)
	{
		// FIX C4244: Explicit cast to float and use float literal
		mx = (cl.mouseDx[0] + cl.mouseDx[1]) * 0.5f;
		my = (cl.mouseDy[0] + cl.mouseDy[1]) * 0.5f;
	}
	else
	{
		// FIX C4244: Explicit cast to float
	...
	if (cl_mouseAccel->value != 0.0f)
	{
		if (cl_mouseAccelStyle->integer == 0)
		{
			...
			// FIX C4244: Explicit cast to float and result of sqrt to float
			rate = (float)sqrt(mx * mx + my * my) / (float)frame_msec;
		...
		else
		{
			...
			// FIX C4244: Explicit cast to float for fabs result
			rate[0] = (float)fabs(mx) / (float)frame_msec;
			rate[1] = (float)fabs(my) / (float)frame_msec;
			// FIX C4244: Explicit cast to float for powf result
			power[0] = (float)powf(rate[0] / cl_mouseAccelOffset->value, cl_mouseAccel->value);
			power[1] = (float)powf(rate[1] / cl_mouseAccelOffset->value, cl_mouseAccel->value);
	...
	if (in_strafe.active)
		// FIX C4244: Explicit cast float calculation to int
...
usercmd_t CL_CreateCmd(void) {
	...
	if (cl_debugMove->integer) {
		if (cl_debugMove->integer == 1) {
			// FIX C4244: Use fabs for float and cast to float for SCR_DebugGraph
			SCR_DebugGraph((float)fabs(cl.viewangles[YAW] - oldAngles[YAW]));
		}
		if (cl_debugMove->integer == 2) {
			// FIX C4244: Use fabs for float and cast to float for SCR_DebugGraph
			SCR_DebugGraph((float)fabs(cl.viewangles[PITCH] - oldAngles[PITCH]));
		}
	}
