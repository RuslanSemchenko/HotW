...
// Fix C4996: Disable deprecation warnings for standard functions
...
void SCR_DrawSmallChar(int x, int y, int ch) {
	...
	// FIX C4244: Explicit cast int to float for DrawStretchPic
...
void SCR_DrawStringExt(int x, int y, float size, const char* string, float* setColor, qboolean forceColor,
	...
	while (*s) {
		...
		// FIX C4244: Explicit cast float to int
		xx += (int)size;
		s++;
	}
...
void SCR_DrawSmallStringExt(int x, int y, const char* string, float* setColor, qboolean forceColor,
	...
	while (*s) {
		...
		// FIX C4244: Explicit cast float macro/val to int
		xx += (int)SMALLCHAR_WIDTH;
		s++;
	}
	re.SetColor(NULL);
}
...
void SCR_DrawDemoRecording(void) {
	...
	// FIX C4996: Use Com_sprintf instead of sprintf
	Com_sprintf(string, sizeof(string), "RECORDING %s: %ik", clc.demoName, pos / 1024);

	// FIX C4267: Cast size_t (from strlen) to int
	SCR_DrawStringExt(320 - (int)strlen(string) * 4, 20, 8, string, g_color_table[7], qtrue, qfalse);
}
...
void SCR_DrawDebugGraph(void)
{
	...
	// FIX C4244: Explicit casts to float
	re.DrawStretchPic((float)x, (float)(y - cl_graphheight->integer),
		(float)w, (float)cl_graphheight->integer, 0, 0, 0, 0, cls.whiteShader);
	re.SetColor(NULL);
...
void SCR_DrawScreenField(stereoFrame_t stereoFrame) {
	...
	if (uiFullscreen || clc.state < CA_LOADING) {
		if (cls.glconfig.vidWidth * 480 > cls.glconfig.vidHeight * 640) {
			re.SetColor(g_color_table[0]);
			// FIX C4244: Explicit casts to float
			re.DrawStretchPic(0.0f, 0.0f, (float)cls.glconfig.vidWidth, (float)cls.glconfig.vidHeight, 0, 0, 0, 0, cls.whiteShader);
			re.SetColor(NULL);
		}
	}
