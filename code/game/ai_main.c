...
#ifndef HotW
	Q_strncpyz(flagstatus, "  ", sizeof(flagstatus));
	if (gametype == GT_CTF) {
		if (BotCTFCarryingFlag(bs)) {
			if (BotTeam(bs) == TEAM_RED) Q_strncpyz(flagstatus, S_COLOR_RED"F ", sizeof(flagstatus)); else Q_strncpyz(flagstatus, S_COLOR_BLUE"F ", sizeof(flagstatus));
		}
	}
#ifdef MISSIONPACK
...
#ifndef HotW
	Q_strncpyz(carrying, "  ", sizeof(carrying));
	if (gametype == GT_CTF) {
		if (BotCTFCarryingFlag(bs))
		{
			Q_strncpyz(carrying, "F ", sizeof(carrying));
		}
	}
#ifdef MISSIONPACK
	else if (gametype == GT_1FCTF)
...
int BotInitLibrary(void) {
	...
	if (!strlen(buf)) Q_strncpyz(buf, "8", sizeof(buf));
	...
	if (!strlen(buf)) Q_strncpyz(buf, "0", sizeof(buf));
