...
static void PM_Footsteps(void) {
	...
	// ИСПРАВЛЕНИЕ: Инициализируем переменную, чтобы избежать ошибки Runtime Check
	bobmove = 0.0f;
...
#else
	if (pml.ladder) {
		// ИСПРАВЛЕНИЕ: Нужно задать bobmove и для лестницы, иначе будет краш
		bobmove = 0.5f; // Используем такое же значение, как при приседании, или подберите своё
