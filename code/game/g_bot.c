...
int G_ParseInfos( char *buf, int max, char *infos[] ) {
	...
	while ( 1 ) {
		...
		len = strlen(info) + strlen("\\num\\") + strlen(va("%d", MAX_ARENAS)) + 1;
		infos[count] = G_Alloc(len);
		if (infos[count]) {
			Q_strncpyz(infos[count], info, len);
			count++;
		}
	}
	return count;
...
void Svcmd_BotList_f( void ) {
	...
	for (i = 0; i < g_numBots; i++) {
		...
		if ( !*name ) {
			Q_strncpyz(name, "UnnamedPlayer", sizeof(name));
		}
		...
		if (!*aifile ) {
			Q_strncpyz(aifile, "bots/default_c.c", sizeof(aifile));
		}
...
static void G_LoadBots( void ) {
	...
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		...
		Q_strncpyz(filename, "scripts/", sizeof(filename));
#else
		Q_strncpyz(filename, "scripts/sg_bots/", sizeof(filename));
#endif
		Q_strcat(filename, sizeof(filename), dirptr);
