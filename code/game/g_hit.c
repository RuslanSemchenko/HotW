...
//	int				filecount = 0;
	...
	if(part == PART_UPPER){
		Q_strncpyz(name, "models/wq3_players/upper.hit", sizeof(name));
	} else if(part == PART_LOWER){
		Q_strncpyz(name, "models/wq3_players/lower.hit", sizeof(name));
	} else
		Q_strncpyz(name, "models/wq3_players/head.hit", sizeof(name));
	...
	for( i = 0; i < header.numMeshes; i++){
		...
		/*if(pos >= len && filecount < 2 && part != PART_HEAD){
			...
			if(filecount == 1){
				if(part == PART_UPPER){
					Q_strncpyz(name, "models/wq3_players/upper_1.hit", sizeof(name));
				} else if (part == PART_LOWER){
					Q_strncpyz(name, "models/wq3_players/lower_1.hit", sizeof(name));
				} else {
				...
				if(part == PART_UPPER){
					Q_strncpyz(name, "models/wq3_players/upper_2.hit", sizeof(name));
				} else if (part == PART_LOWER){
					Q_strncpyz(name, "models/wq3_players/lower_2.hit", sizeof(name));
				} else {
					G_Printf("no valid operation\n");
					return qfalse;
				}
