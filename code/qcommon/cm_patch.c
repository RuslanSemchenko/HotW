...
static qboolean	CM_NeedsSubdivision(vec3_t a, vec3_t b, vec3_t c) {
	...
	for (i = 0; i < 3; i++) {
		// FIX C4244: Explicit cast double to vec_t
...
void CM_AddFacetBevels(facet_t* facet) {
	...
	for (axis = 0; axis < 3; axis++)
	{
		...
		{
			VectorClear(plane);
			// FIX C4244: Explicit cast int to float
	...
	for (j = 0; j < w->numpoints; j++)
	{
		...
		CM_SnapVector(vec);
		for (k = 0; k < 3; k++)
			// FIX C4244: Explicit cast int to vec_t
			if (vec[k] == (vec_t)-1 || vec[k] == (vec_t)1)
				break;	// axial
		if (k < 3)
...
void CM_TracePointThroughPatchCollide(traceWork_t* tw, const struct patchCollide_s* pc) {
	...
	float		intersect;
	const patchPlane_t* patchPlanes; // Renamed to avoid hiding global 'planes'
	const facet_t* facet;
	int			i, j, k;
	float		offset;
	float		d1, d2;
	...
	patchPlanes = pc->planes;
	for (i = 0; i < pc->numPlanes; i++, patchPlanes++) {
		offset = DotProduct(tw->offsets[patchPlanes->signbits], patchPlanes->plane);
		d1 = DotProduct(tw->start, patchPlanes->plane) - patchPlanes->plane[3] + offset;
		d2 = DotProduct(tw->end, patchPlanes->plane) - patchPlanes->plane[3] + offset;
		if (d1 <= 0) {
		...
		if (d1 == d2) {
			...
			// FIX C4244: Explicit cast double to float
			intersection[i] = (float)(d1 / (d1 - d2));
			if (intersection[i] <= 0) {
				intersection[i] = 99999;
			}
		}
	...
	for (i = 0; i < pc->numFacets; i++, facet++) {
		...
		if (j == facet->numBorders) {
...
#endif //BSPC
			patchPlanes = &pc->planes[facet->surfacePlane];
			...
			d2 = DotProduct(tw->end, patchPlanes->plane) - patchPlanes->plane[3] + offset;
			// FIX C4244: Explicit cast double to float
			tw->trace.fraction = (float)((d1 - SURFACE_CLIP_EPSILON) / (d1 - d2));

			if (tw->trace.fraction < 0) {
				tw->trace.fraction = 0;
			}

			VectorCopy(patchPlanes->plane, tw->trace.plane.normal);
			tw->trace.plane.dist = patchPlanes->plane[3];
		}
	}
}
...
void CM_TraceThroughPatchCollide(traceWork_t* tw, const struct patchCollide_s* pc) {
	...
	patchPlane_t* patchPlanes; // Renamed to avoid hiding global 'planes'
	facet_t* facet;
	...
	for (i = 0; i < pc->numFacets; i++, facet++) {
		...
		//
		patchPlanes = &pc->planes[facet->surfacePlane];
		VectorCopy(patchPlanes->plane, plane);
		plane[3] = patchPlanes->plane[3];
		if (tw->sphere.use) {
			...
			offset = DotProduct(tw->offsets[patchPlanes->signbits], plane);
			plane[3] -= offset;
		...
		for (j = 0; j < facet->numBorders; j++) {
			patchPlanes = &pc->planes[facet->borderPlanes[j]];
			if (facet->borderInward[j]) {
				VectorNegate(patchPlanes->plane, plane);
				plane[3] = -patchPlanes->plane[3];
			}
			else {
				VectorCopy(patchPlanes->plane, plane);
...
void CM_DrawDebugSurface(void (*drawPoly)(int color, int numPoints, float* points)) {
	...
	for (i = 0, facet = pc->facets; i < pc->numFacets; i++, facet++) {
		...
		for (k = 0; k < facet->numBorders + 1; k++) {
			...
			// FIX C4244: Explicit cast double to float
			plane[3] += (float)fabs(DotProduct(v1, v2));
			//*/
