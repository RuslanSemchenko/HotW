...
void CM_TestCapsuleInCapsule(traceWork_t * tw, clipHandle_t model) {
	...
	for (i = 0; i < 3; i++) {
		// FIX C4244: Explicit cast double to vec_t (float)
		offset[i] = (vec_t)((mins[i] + maxs[i]) * 0.5);
...
void CM_TraceThroughSphere(traceWork_t * tw, vec3_t origin, float radius, vec3_t start, vec3_t end) {
	...
	if (d > 0) {
		...
		// = (- b + sqrtd) * 0.5f; // / (2.0f * a);
		// FIX C4244: Explicit cast double to float
		fraction = (float)((-b - sqrtd) * 0.5f); // / (2.0f * a);
		//
		if (fraction < 0) {
			fraction = 0;
		}
...
void CM_TraceThroughVerticalCylinder(traceWork_t * tw, vec3_t origin, float radius, float halfheight, vec3_t start, vec3_t end) {
	...
	// 2d coordinates
	// FIX C4244: Explicit cast double to vec_t (float)
	VectorSet(start2d, (vec_t)start[0], (vec_t)start[1], 0);
	VectorSet(end2d, (vec_t)end[0], (vec_t)end[1], 0);
	VectorSet(org2d, (vec_t)origin[0], (vec_t)origin[1], 0);
	...
	if (tw.sphere.use) {
		for (i = 0; i < 3; i++) {
			if (tw.start[i] < tw.end[i]) {
				// FIX C4244: Explicit cast double to vec_t (float)
				tw.bounds[0][i] = (vec_t)(tw.start[i] - fabs(tw.sphere.offset[i]) - tw.sphere.radius);
				// FIX C4244: Explicit cast double to vec_t (float)
				tw.bounds[1][i] = (vec_t)(tw.end[i] + fabs(tw.sphere.offset[i]) + tw.sphere.radius);
			}
			else {
				// FIX C4244: Explicit cast double to vec_t (float)
				tw.bounds[0][i] = (vec_t)(tw.end[i] - fabs(tw.sphere.offset[i]) - tw.sphere.radius);
				// FIX C4244: Explicit cast double to vec_t (float)
				tw.bounds[1][i] = (vec_t)(tw.start[i] + fabs(tw.sphere.offset[i]) + tw.sphere.radius);
			}
		}
	}
	else {
