...
void QDECL Com_Error(int code, const char* fmt, ...) {
	...
	if (code == ERR_DISCONNECT || code == ERR_SERVERDISCONNECT) {
		// ИСПРАВЛЕНИЕ: Не выгружаем DLL перед longjmp на x64!
		...
		// ИСПРАВЛЕНИЕ: Не выгружаем DLL перед longjmp на x64!
		// Это предотвращает ошибку 0xC00000FF (Bad Function Table),
		// так как стек вызовов остается валидным для раскрутки.
		// VM_Forced_Unload_Start(); 
		...
		// Здесь можно оставить как есть или тоже закомментировать, 
		// если краш повторится при проверке CD-ключа.
		VM_Forced_Unload_Start();
		...
		// ERR_FATAL - здесь мы выходим из игры полностью, поэтому выгрузка безопасна (мы не делаем longjmp)
		VM_Forced_Unload_Start();
...
void Com_AppendCDKey(const char* filename) {
	...
	if (CL_CDKeyValidate(buffer, NULL)) {
		Q_strcat(cl_cdkey, sizeof(cl_cdkey), buffer);
	}
	else {
		Q_strncpyz(&cl_cdkey[16], "                ", 17);
	}
...
void Com_Init(char* commandLine) {
	...
	//
	// ИЗМЕНЕНИЕ: sv_pure теперь 0 по умолчанию (отключен)
	Cvar_Get("sv_pure", "0", CVAR_SERVERINFO | CVAR_LATCH);
	...
	// ИЗМЕНЕНИЕ: VM_GAME, VM_CGAME, VM_UI теперь 0 по умолчанию (DLL вместо QVM)
	Cvar_Get("vm_game", "0", CVAR_LATCH);
	Cvar_Get("vm_cgame", "0", CVAR_LATCH);
	Cvar_Get("vm_ui", "0", CVAR_LATCH);
...
void Com_ReadFromPipe(void)
{
	...
	while (1) {
		len = sizeof(buf) - accu - 1;
		if (len <= 0) {
			// buffer is full and no newline, process it
			if (accu > 0) {
				Cbuf_ExecuteText(EXEC_APPEND, buf);
			}
			accu = 0;
			continue; // try to read again with a clear buffer
		}

		read = FS_Read(buf + accu, len, pipefile);
		if (read <= 0) {
