...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename)
{
	...
	if ( (uLong)sizeof(fileInPack_t) > (uLong)INT_MAX / gi.number_entry ||
		(gi.number_entry * sizeof(fileInPack_t)) > (uLong)INT_MAX - len ) {
	...
	for (i = 0; i < gi.number_entry; i++)
	{
		...
		Q_strncpyz( buildBuffer[i].name, filename_inzip, strlen(filename_inzip) + 1 );
		...
		if (Q_stricmp(name, com_basegame->string) && Q_stricmpn(name, ".", 1)) {
			...
			if (nPaks > 0) {
				...
				if ( nDescLen > 0 && descHandle) {
					...
					nDescLen = fread(descPath, 1, sizeof(descPath) - 1, file);
					if (nDescLen >= 0) {
						descPath[nDescLen] = '\0';
					}
