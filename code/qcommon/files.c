...
qboolean FS_IsExt(const char* filename, const char* ext, int namelen)
{
	...
	if (namelen < 0 || !filename || !ext)
	{
		return qfalse;
	}
	filelen = (size_t)namelen;
	extlen = strlen(ext);
	if (extlen > filelen || Q_stricmp(filename + filelen - extlen, ext))
	{
		return qfalse;
	}
	return qtrue;
...
int FS_Read2(void* buffer, int len, fileHandle_t f) {
	if (!fs_searchpaths) { // FIXME: This check is redundant, FS_Read does it too.
		Com_Error(ERR_FATAL, "Filesystem call made without initialization");
	}
...
int FS_Read(void* buffer, int len, fileHandle_t f) {
	...
	if (len < 0) {
		// negative length is invalid, return 0 bytes read
		return 0;
	}
...
int FS_Seek(fileHandle_t f, long offset, int origin) {
	...
	if (fsh[f].zipFile == qtrue) {
		...
		case FS_SEEK_END:
		case FS_SEEK_CUR:
			while (remainder > PK3_SEEK_BUFFER_SIZE) { // FIXME: size_t
...
static pack_t* FS_LoadZipFile(const char* zipfile, const char* basename)
{
	...
	for (i = 0; i < gi.number_entry; i++)
	{
		...
		len += (size_t)strlen(filename_inzip) + 1;   // безопасно: size_t
		unzGoToNextFile(uf);
	}

	// check for integer overflow before memory allocation
	if ((uLong)sizeof(fileInPack_t) > (uLong)INT_MAX / gi.number_entry ||
		(gi.number_entry * sizeof(fileInPack_t)) > (size_t)INT_MAX - len) {
		Com_Printf("Warning: The zip file %s is too big, cannot load.\n", zipfile);
		unzClose(uf);
		return NULL;
	}

	// check for integer overflow before memory allocation
	if ((uLong)sizeof(int) > (uLong)INT_MAX / (gi.number_entry + 1)) {
		Com_Printf("Warning: The zip file %s has too many files, cannot load.\n", zipfile);
		unzClose(uf);
		return NULL;
	}

	// вычисляем общий размер как size_t
	size_t allocSize = (size_t)gi.number_entry * sizeof(fileInPack_t) + len;
	buildBuffer = Z_Malloc((int)allocSize);   // явное приведение к int
	...
	pack = Z_Malloc((int)(sizeof(pack_t) + i * sizeof(fileInPack_t*)));
	pack->hashSize = i;
	...
	for (i = 0; i < gi.number_entry; i++)
	{
		...
		size_t nameLen = strlen(filename_inzip);
		Q_strncpyz(buildBuffer[i].name, filename_inzip, (int)(nameLen + 1)); // явное приведение
		namePtr += nameLen + 1;                                             // size_t
...
static int FS_ReturnPath(const char* zname, char* zpath, int* depth) {
	...
	Q_strncpyz(zpath, zname, sizeof(zpath));
	zpath[len] = 0;
	*depth = newdep;
...
char** FS_ListFilteredFiles(const char* path, const char* extension, char* filter, int* numfiles, qboolean allowNonPureFilesOnDisk) {
		...
		if (search->pack) {
			...
			for (i = 0; i < pak->numfiles; i++) {
				...
				if (filter) {
					...
					length = strlen(name);
					if (length < (size_t)extensionLength) {
						continue;
					}
					if (Q_stricmp(name + length - extensionLength, extension)) {
						continue;
					}
					temp = (size_t)pathLength;
					if (pathLength) {
						temp++;
					}
	...
	*numfiles = nfiles;
	...
	listCopy = Z_Malloc((int)((nfiles + 1) * sizeof(*listCopy)));
	for (i = 0; i < nfiles; i++) {
		listCopy[i] = list[i];
	}
	listCopy[i] = NULL;
...
int	FS_GetFileList(const char* path, const char* extension, char* listbuf, int bufsize) {
	...
	*listbuf = 0;
	...
	for (i = 0; i < nFiles; i++) {
		...
		if (nTotal + (int)nLen + 1 < bufsize) {
			Q_strncpyz(listbuf, pFiles[i], (size_t)bufsize - nTotal);
			listbuf += nLen;
			nTotal += (int)nLen;
		}
		else {
...
int FS_GetModList(char* listbuf, int bufsize) {
	int     nMods, i, j, nPaks, nPotential;
	size_t  nTotal;
	...
	*listbuf = 0;
	...
	pFiles0 = Sys_ListFiles(fs_homepath->string, NULL, NULL, &nPotential, qtrue);
	pFiles1 = Sys_ListFiles(fs_basepath->string, NULL, NULL, &nPotential, qtrue);
	...
	for (i = 0; i < nPotential; i++) {
		...
		if (Q_stricmp(name, com_basegame->string) && Q_stricmpn(name, ".", 1)) {
			...
			if (nPaks > 0) {
				...
				int filelen = (int)FS_SV_FOpenFileRead(descPath, &descHandle);
				if (filelen > 0 && descHandle) {
					FILE* file;
					file = FS_FileForHandle(descHandle);
					Com_Memset(descPath, 0, sizeof(descPath));
					int readlen = (int)fread(descPath, 1, sizeof(descPath) - 1, file);
					if (readlen >= 0) {
						descPath[readlen] = '\0';
					}
				...
				descLen = strlen(descPath) + 1;

				if (nTotal + nameLen + 1 + descLen + 1 < (size_t)bufsize) {
					Q_strncpyz(listbuf, name, bufsize - (int)nTotal);
					listbuf += nameLen;
					Q_strncpyz(listbuf, descPath, bufsize - (int)(nTotal + nameLen));
					listbuf += descLen;
...
void FS_AddGameDirectory(const char* path, const char* dir) {
	...
	searchpath_t* search;
	pack_t* pak;
	char            curpath[MAX_OSPATH], * pakfile;
	int             numfiles;
	char** pakfiles;
	int             pakfilesi, pakwhich;
	size_t          len;
	char** pakfilestmp;
	int             numdirs;
	char** pakdirs;
	...
	while ((pakfilesi < numfiles) || (pakdirsi < numdirs))
	{
		...
		if (pakwhich) {
			...
			if (!FS_IsExt(pakdirs[pakdirsi], ".pk3dir", (int)len)) {
				pakdirsi++;
				continue;
			}
...
const char* FS_ReferencedPakChecksums(void) {
	...
	for (search = fs_searchpaths; search; search = search->next) {
		if (search->pack) {
			if (search->pack->referenced || Q_stricmpn(search->pack->pakGamename, com_basegame->string, (int)strlen(com_basegame->string))) {
				Q_strcat(info, sizeof(info), va("%i ", search->pack->checksum));
			}
		}
	}
