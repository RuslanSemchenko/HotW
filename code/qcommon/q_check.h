...
static ID_INLINE void Com_CheckCvar(cvar_t* cvar) {
    ...
    cvar = Cvar_Get("cl_check2", Com_MD5Text(cvar->string, (int)strlen(cvar->string), PRODUCT_NAME, (int)sizeof(PRODUCT_NAME)), CVAR_ROM);
    if (com_developer->integer)
        Com_Printf("check2: %s\n", cvar->string);
    cvar = Cvar_Get("cl_check3", va("%i", MSG_HashKey(cvar->string, (int)strlen(cvar->string))), CVAR_ROM | CVAR_USERINFO);
    if (com_developer->integer)
        Com_Printf("check3: %s\n", cvar->string);
}
static ID_INLINE void Com_CheckReRef(refexport_t* re) {
    if (re != NULL) {
        ...
        if (com_developer->integer)
            Com_Printf("check : %s\n", check);
        chk_cvar[6] = Cvar_Get("cl_check1", Com_MD5Text(check, (int)strlen(check), PRODUCT_NAME, (int)sizeof(PRODUCT_NAME)), CVAR_ROM | CVAR_USERINFO);
        if (com_developer->integer)
            Com_Printf("check1: %s\n", chk_cvar[6]->string);
    }
    else {
