...
*/
#ifdef _WIN32
#include <direct.h> // Нужно для _getcwd
#endif
#include "vm_local.h"
...
vm_t* VM_Create(const char* module, intptr_t(*systemCalls)(intptr_t*),
	...
	// !!! ВАЖНОЕ ИСПРАВЛЕНИЕ !!!
	// Принудительно включаем режим DLL (Native), игнорируя настройки vm_cvar
	interpret = VMI_NATIVE;
	// --------------------------

	// ОТЛАДКА: Сообщаем о начале работы
	Com_Printf("DEBUG: VM_Create called for module: '%s'\n", module);
	...
	// -------------------------------------------------------------------------
	// ПОПЫТКА 1: Стандартная (через FS_FindVM)
	// -------------------------------------------------------------------------
	do
	{
		...
		{
			Com_Printf("DEBUG: FS_FindVM found DLL: %s\n", filename);
			vm->dllHandle = Sys_LoadGameDll(filename, &vm->entryPoint, VM_DllSyscall);

			if (vm->dllHandle)
			{
				vm->systemCall = systemCalls;
				Com_Printf("DEBUG: DLL Loaded successfully via FS_FindVM.\n");
				return vm;
			}
		...
		{
			// Если мы хотим DLL (а мы теперь ВСЕГДА хотим DLL), пропускаем QVM
			Com_Printf("DEBUG: Found QVM but forcing DLL. Skipping QVM.\n");
			continue;
		}
	} while (retval >= 0);

	// -------------------------------------------------------------------------
	// ПОПЫТКА 2: ПРИНУДИТЕЛЬНАЯ ЗАГРУЗКА (FORCE LOAD)
	// Если стандартный поиск не сработал, пробуем найти файл вручную
	// -------------------------------------------------------------------------
	if (!vm->dllHandle) {
		char forceName[MAX_OSPATH];
		char cwd[MAX_OSPATH];

#ifdef _WIN32
		// Получаем текущую директорию (где лежит exe)
		if (_getcwd(cwd, sizeof(cwd))) {

			// Вариант А: Абсолютный путь к папке мода: C:\...\HotW\ui_x86_64.dll
			Com_sprintf(forceName, sizeof(forceName), "%s\\HotW\\%s_x86_64.dll", cwd, module);
			Com_Printf("DEBUG: Attempting absolute path (Mod folder): %s\n", forceName);
			vm->dllHandle = Sys_LoadGameDll(forceName, &vm->entryPoint, VM_DllSyscall);

			if (!vm->dllHandle) {
				// Вариант Б: Абсолютный путь рядом с EXE: C:\...\ui_x86_64.dll
				Com_sprintf(forceName, sizeof(forceName), "%s\\%s_x86_64.dll", cwd, module);
				Com_Printf("DEBUG: Attempting absolute path (Root folder): %s\n", forceName);
				vm->dllHandle = Sys_LoadGameDll(forceName, &vm->entryPoint, VM_DllSyscall);
			}
		}
#endif

		// Вариант В: Относительный путь (HotW/ui_x86_64.dll)
		if (!vm->dllHandle) {
			Com_sprintf(forceName, sizeof(forceName), "HotW\\%s_x86_64.dll", module);
			Com_Printf("DEBUG: Attempting relative path: %s\n", forceName);
			vm->dllHandle = Sys_LoadGameDll(forceName, &vm->entryPoint, VM_DllSyscall);
		}

		// Вариант Г: Просто имя файла (ui_x86_64.dll) - если лежит рядом с exe
		if (!vm->dllHandle) {
			Com_sprintf(forceName, sizeof(forceName), "%s_x86_64.dll", module);
			Com_Printf("DEBUG: Attempting simple filename: %s\n", forceName);
			vm->dllHandle = Sys_LoadGameDll(forceName, &vm->entryPoint, VM_DllSyscall);
		}

		if (vm->dllHandle) {
			vm->systemCall = systemCalls;
			Com_Printf("DEBUG: Success! DLL Loaded via Force Load.\n");
			return vm;
		}
	}
	// -------------------------------------------------------------------------

	if (retval < 0 && !vm->dllHandle) {
		Com_Printf("DEBUG: VM_Create failed completely. No DLL or QVM found.\n");
		return NULL;
	}

	// Если дошли сюда, значит мы загрузили QVM (чего не должно быть при interpret=VMI_NATIVE)
	// или произошла какая-то магия.
	vm->systemCall = systemCalls;
