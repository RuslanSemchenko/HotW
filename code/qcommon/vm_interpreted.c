...
void VM_PrepareInterpreter(vm_t* vm, vmHeader_t* header) {
	...
	while (instruction < header->instructionCount) {
		...
		case OP_GEF:
			...
			// FIX C4244: Explicit cast from intptr_t (or ptr) to int, assuming 32-bit VM addressing
...
#define r2 codeImage[programCounter]
	...
	nextInstruction2:
		...
		case OP_STORE2:
			// FIX C4244: Explicit cast to short
			*(short*)&image[r1 & (dataMask & ~1)] = (short)r0;
			opStackOfs -= 2;
			goto nextInstruction;
		case OP_STORE1:
			// FIX C4244: Explicit cast to byte (or char)
...
//				temp = vm->callLevel;
				...
				* (int*)&image[programStack + 4] = -1 - programCounter;
				...
				//VM_LogSyscalls( (int *)&image[ programStack + 4 ] );
				{
					...
					if (sizeof(intptr_t) != sizeof(int)) {
						...
						// FIX C4244: Explicit cast return value from intptr_t to int
						r = (int)vm->systemCall(argarr);
					}
					else {
						intptr_t* argptr = (intptr_t*)&image[programStack + 4];
						// FIX C4244: Explicit cast return value from intptr_t to int
						r = (int)vm->systemCall(argptr);
					}
				}
			...
			else if ((unsigned)programCounter >= (unsigned)vm->instructionCount) { // FIX C4018
		...
		case OP_LEAVE:
			...
			else if ((unsigned)programCounter >= (unsigned)vm->codeLength) { // FIX C4018
		...
		case OP_JUMP:
			if ((unsigned)r0 >= (unsigned)vm->instructionCount) // FIX C4018
			{
