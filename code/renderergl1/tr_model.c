...
=================
R_LoadAssimp
Загружает модель через Assimp и конвертирует её в формат MD3 в памяти.
=================
*/
static qboolean R_LoadAssimp(model_t* mod, void* buffer, int bufferSize, const char* name) {
	const struct aiScene* scene;
	unsigned int i, j;
	int flags;

	// Переменные для расчета размеров
	int totalSize = 0;
	int numSurfaces = 0;
	int offsetFrames, offsetTags, offsetSurfaces, offsetEnd;

	// Указатели для записи данных
	byte* data;
	md3Header_t* header;
	md3Frame_t* frame;
	md3Surface_t* surf;
	...
	// Флаги импорта
	flags = aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_GenNormals | aiProcess_JoinIdenticalVertices | aiProcess_CalcTangentSpace;

	// Загружаем сцену из памяти
	scene = aiImportFileFromMemory((const char*)buffer, bufferSize, flags, "");

	if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) {
		const char* err = aiGetErrorString();
		ri.Printf(PRINT_WARNING, "Assimp load error for %s: %s\n", name, err ? err : "Unknown error");
		return qfalse;
	}

	numSurfaces = scene->mNumMeshes;
	if (numSurfaces == 0) {
		aiReleaseImport(scene);
		return qfalse;
	}

	// 1. РАСЧЕТ РАЗМЕРА ПАМЯТИ
	// Структура MD3: Header -> Frames -> Tags -> Surfaces

	totalSize = sizeof(md3Header_t);

	// Frames (1 кадр)
	offsetFrames = totalSize;
	totalSize += sizeof(md3Frame_t);

	// Tags (0)
	offsetTags = totalSize;

	// Surfaces
	offsetSurfaces = totalSize;
	for (i = 0; i < numSurfaces; i++) {
		struct aiMesh* mesh = scene->mMeshes[i];
		int surfSize = sizeof(md3Surface_t);

		// Shaders (1 шейдер на поверхность)
		surfSize += sizeof(md3Shader_t);

		// Triangles
		surfSize += mesh->mNumFaces * sizeof(md3Triangle_t);

		// TexCoords (ST)
		surfSize += mesh->mNumVertices * sizeof(md3St_t);

		// XYZ + Normals
		surfSize += mesh->mNumVertices * sizeof(md3XyzNormal_t);
	...
	// Выделяем один большой блок памяти для всей модели
	mod->md3[0] = (md3Header_t*)ri.Hunk_Alloc(totalSize, h_low);

	// Обнуляем память
	Com_Memset(mod->md3[0], 0, totalSize);
	data = (byte*)mod->md3[0];
	...
	header->numSkins = 0;
	header->ofsFrames = offsetFrames;
	header->ofsTags = offsetTags;
	header->ofsSurfaces = offsetSurfaces;
	header->ofsEnd = offsetEnd;

	// 4. ЗАПОЛНЕНИЕ ПОВЕРХНОСТЕЙ
	// Указатель на начало первой поверхности
	surf = (md3Surface_t*)(data + offsetSurfaces);
	...
	for (i = 0; i < numSurfaces; i++) {
		...
		int currentSurfSize = sizeof(md3Surface_t);
		int ofsShaders = currentSurfSize;
		currentSurfSize += sizeof(md3Shader_t);

		int ofsTriangles = currentSurfSize;
		currentSurfSize += mesh->mNumFaces * sizeof(md3Triangle_t);

		int ofsSt = currentSurfSize;
		currentSurfSize += mesh->mNumVertices * sizeof(md3St_t);

		int ofsXyzNormals = currentSurfSize;
		currentSurfSize += mesh->mNumVertices * sizeof(md3XyzNormal_t);

		int ofsEnd = currentSurfSize;

		// Заполняем заголовок поверхности
		surf->ident = SF_MD3;
		Q_strncpyz(surf->name, mesh->mName.data, sizeof(surf->name));
		surf->flags = 0;
		surf->numFrames = 1;
		surf->numShaders = 1;
		surf->numVerts = mesh->mNumVertices;
		surf->numTriangles = mesh->mNumFaces;
		surf->ofsTriangles = ofsTriangles;
		surf->ofsShaders = ofsShaders;
		surf->ofsSt = ofsSt;
		surf->ofsXyzNormals = ofsXyzNormals;
		surf->ofsEnd = ofsEnd;

		// Шейдер
		shader = (md3Shader_t*)((byte*)surf + ofsShaders);
		// Пытаемся найти имя текстуры
		if (scene->mMaterials && mesh->mMaterialIndex < scene->mNumMaterials) {
			...
			if (aiGetMaterialTexture(mat, aiTextureType_DIFFUSE, 0, &path, NULL, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
				...
				Q_strncpyz(texturePath, path.data, sizeof(texturePath));
				COM_StripExtension(texturePath, shader->name, sizeof(shader->name));
			}
			else {
				// Если текстуры нет, используем имя модели как базу
		...
		// Треугольники
		tri = (md3Triangle_t*)((byte*)surf + ofsTriangles);
		for (j = 0; j < mesh->mNumFaces; j++) {
			struct aiFace face = mesh->mFaces[j];
			if (face.mNumIndices == 3) {
				tri[j].indexes[0] = face.mIndices[0];
				tri[j].indexes[1] = face.mIndices[1];
				tri[j].indexes[2] = face.mIndices[2];
			}
		}

		// Текстурные координаты
		st = (md3St_t*)((byte*)surf + ofsSt);
		for (j = 0; j < mesh->mNumVertices; j++) {
			if (mesh->mTextureCoords[0]) {
				st[j].st[0] = mesh->mTextureCoords[0][j].x;
				st[j].st[1] = mesh->mTextureCoords[0][j].y;
			}
			else {
				st[j].st[0] = 0; st[j].st[1] = 0;
			}
		}

		// Координаты и Нормали
		xyz = (md3XyzNormal_t*)((byte*)surf + ofsXyzNormals);
		for (j = 0; j < mesh->mNumVertices; j++) {
			float x = mesh->mVertices[j].x;
			float y = mesh->mVertices[j].y;
			float z = mesh->mVertices[j].z;

			// MD3 хранит координаты в short (умноженные на 64)
			xyz[j].xyz[0] = (short)(x * 64.0f);
			xyz[j].xyz[1] = (short)(y * 64.0f);
			xyz[j].xyz[2] = (short)(z * 64.0f);

			// Обновляем границы (Bounding Box) для кадра
			...
			// Нормали
			xyz[j].normal = 0; // Пока без нормалей
		}

		// Переходим к следующей поверхности
		surf = (md3Surface_t*)((byte*)surf + ofsEnd);
	}

	// 5. ЗАПОЛНЕНИЕ КАДРА
	frame = (md3Frame_t*)(data + offsetFrames);
	VectorCopy(mins, frame->bounds[0]);
	VectorCopy(maxs, frame->bounds[1]);
	VectorClear(frame->localOrigin);
	frame->radius = RadiusFromBounds(mins, maxs);
	Q_strncpyz(frame->name, "default", sizeof(frame->name));

	aiReleaseImport(scene);

	ri.Printf(PRINT_ALL, "Assimp loaded %s successfully (Meshes: %d)\n", name, numSurfaces);
	return qtrue;
}
...
====================
R_RegisterAssimp
Обертка для загрузки форматов Assimp (obj, fbx, gltf...)
====================
*/
qhandle_t R_RegisterAssimp(const char* name, model_t* mod)
{
	...
	if (!buffer) {
		// ri.Printf(PRINT_DEVELOPER, "R_RegisterAssimp: File not found: %s\n", name);
		mod->type = MOD_BAD;
		return 0;
	}

	loaded = R_LoadAssimp(mod, buffer, filesize, name);
...
void R_GetAnimTag(mdrHeader_t* mod, int framenum, const char* tagName, md3Tag_t* dest)
{
	...
	Q_strncpyz(dest->name, "", sizeof(dest->name));
}
