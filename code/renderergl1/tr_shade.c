...
static void ProjectDlightTexture_scalar(void) {
	...
	// Переносим тяжелые массивы из стека в статическую память
	...
	static unsigned int	hitIndexes[SHADER_MAX_INDEXES];
	...
	for (l = 0; l < backEnd.refdef.num_dlights; l++) {
		...
		if (!(tess.dlightBits & (1 << l))) {
			continue;	// эта поверхность точно не освещается этим dlight
		}
		texCoords = texCoordsArray[0];
		colors = colorArray[0];
		...
		qglEnableClientState(GL_COLOR_ARRAY);
		qglColorPointer(4, GL_UNSIGNED_BYTE, 0, colorArray[0]);
