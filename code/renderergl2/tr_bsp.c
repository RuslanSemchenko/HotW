// tr_bsp.c -- functions that are not called every frame

#include "tr_local.h"
#ifdef HotW
#include "../qcommon/q_check.h"
#endif
#include <stdio.h> // For sscanf_s, sprintf_s
...
void R_LoadEntities(lump_t* l) {
	...
	while (1) {
		...
		if (!Q_stricmp(keyname, "gridsize")) {
			sscanf_s(value, "%f %f %f", &w->lightGridSize[0], &w->lightGridSize[1], &w->lightGridSize[2]);
			continue;
		}

		// check for auto exposure
		if (!Q_stricmp(keyname, "autoExposureMinMax")) {
			sscanf_s(value, "%f %f", &tr.autoExposureMinMax[0], &tr.autoExposureMinMax[1]);
			continue;
		}
	}
}
...
qboolean R_ParseSpawnVars(char* spawnVarChars, int maxSpawnVarChars, int* numSpawnVars, char* spawnVars[MAX_SPAWN_VARS][2])
{
	...
	*numSpawnVars = 0;
	...
	while (1) {
		...
		keyLength = (int)strlen(keyname) + 1;
		tokenLength = (int)strlen(com_token) + 1;
		...
		Q_strncpyz(spawnVarChars + numSpawnVarChars, keyname, sizeof(spawnVarChars) - numSpawnVarChars);
		spawnVars[*numSpawnVars][0] = spawnVarChars + numSpawnVarChars;
		numSpawnVarChars += keyLength;

		Q_strncpyz(spawnVarChars + numSpawnVarChars, com_token, sizeof(spawnVarChars) - numSpawnVarChars);
...
void R_LoadCubemapEntities(char* cubemapEntityName)
{
	...
	while (R_ParseSpawnVars(spawnVarChars, sizeof(spawnVarChars), &numSpawnVars, spawnVars))
	{
		...
		for (i = 0; i < numSpawnVars; i++)
		{
			...
			{
				sscanf_s(spawnVars[i][1], "%f %f %f", &origin[0], &origin[1], &origin[2]);
				positionSet = qtrue;
			}
		}
...
void RE_LoadWorldMap(const char* name) {
	...
	lg_i = LittleLong(header->version);
	if (lg_i != BSP_VERSION) {
		ri.Error(ERR_DROP, "RE_LoadWorldMap: %s has wrong version number (%i should be %i)",
			name, lg_i, BSP_VERSION);
	}

	// swap all the lumps
	for (lg_i = 0; lg_i < sizeof(dheader_t) / 4; lg_i++) {
		((int*)header)[lg_i] = LittleLong(((int*)header)[lg_i]);
	}
	...
	if (0)
	{
		...
		{
			int tga_i;
			byte* tgaBuffer = ri.Malloc(w->lightGridBounds[0] * w->lightGridBounds[1] * 3 + 18);
			byte* out;
			uint8_t* in;
			char fileName[MAX_QPATH];

			Com_Memset(tgaBuffer, 0, 18);
			tgaBuffer[2] = 2;		// uncompressed type
			tgaBuffer[12] = w->lightGridBounds[0] & 255;
			tgaBuffer[13] = w->lightGridBounds[0] >> 8;
			tgaBuffer[14] = w->lightGridBounds[1] & 255;
			tgaBuffer[15] = w->lightGridBounds[1] >> 8;
			tgaBuffer[16] = 24;	// pixel size

			in = primaryLightGrid;
			for (tga_i = 0; tga_i < w->lightGridBounds[2]; tga_i++)
			{
				int j;

				sprintf_s(fileName, sizeof(fileName), "primarylg%d.tga", tga_i);
				...
				ri.FS_WriteFile(fileName, tgaBuffer, w->lightGridBounds[0] * w->lightGridBounds[1] * 3 + 18);
			}
