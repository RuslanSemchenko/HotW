...
byte* RB_ReadPixels(int x, int y, int width, int height, size_t* offset, int* padlen)
{
	...
	// Исправлено warning C4267: size_t -> int
...
const void* RB_TakeVideoFrameCmd(const void* data)
{
	...
	// Исправлено warning C4267: size_t -> int
	padwidth = (int)PAD(linelen, packAlign);
	padlen = padwidth - (int)linelen;
	// AVI line padding
	// Исправлено warning C4267: size_t -> int
	avipadwidth = (int)PAD(linelen, AVI_LINE_PADDING);
	...
	if (cmd->motionJpeg)
	{
		memcount = RE_SaveJPGToBuffer(cmd->encodeBuffer, (int)(linelen * cmd->height),
