...
static void ScanAndLoadShaderFiles( void )
{
	...
	for ( i = 0; i < numShaderFiles; i++ )
	{
		...
		// look for a .mtr file first
		{
			...
			{
				Q_strncpyz(ext, ".mtr", sizeof(filename) - (ext - filename));
			}
	...
	p = s_shaderText;
	size_t remaining_size = sum + numShaderFiles*2;
	...
	for ( i = numShaderFiles - 1; i >= 0 ; i-- ) {
		...
		int len = strlen(buffers[i]);
		if (len + 1 >= remaining_size) { // +1 for newline
			ri.Printf(PRINT_WARNING, "Shader text buffer overflow\n");
			break;
		}
		memcpy(p, buffers[i], len);
		p += len;
		*p++ = '\n';
