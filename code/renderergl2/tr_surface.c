...
static void RB_SurfaceSprite(void) {
	...
	if (ent->e.rotation == 0) {
		...
		ang = (float)(M_PI * ent->e.rotation / 180.0f);
		s = sinf(ang);
		c = cosf(ang);
...
static qboolean RB_SurfaceVbo(VBO_t* vbo, IBO_t* ibo, int numVerts, int numIndexes, int firstIndex, int minIndex, int maxIndex, int dlightBits, int pshadowBits, qboolean shaderCheck)
{
	...
	if (r_mergeMultidraws->integer)
	{
		...
		for (; i < (int)tess.multiDrawPrimitives; i++)
		{
	...
	else if (mergeBack != -1 && mergeForward != -1)
	{
		...
		if (mergeForward != (int)tess.multiDrawPrimitives)
		{
...
#define NUM_BEAM_SEGS 6
	...
	{
		RotatePointAroundVector(start_points[i], normalized_direction, perpvec, (float)((360.0f / NUM_BEAM_SEGS) * i));
...
static void DoRailCore(const vec3_t start, const vec3_t end, const vec3_t up, float len, float spanWidth)
{
	...
	tess.vertexColors[tess.numVertexes][0] = (vec_t)(backEnd.currentEntity->e.shaderRGBA[0] * 0.25f / 255.0f);
	tess.vertexColors[tess.numVertexes][1] = (vec_t)(backEnd.currentEntity->e.shaderRGBA[1] * 0.25f / 255.0f);
	tess.vertexColors[tess.numVertexes][2] = (vec_t)(backEnd.currentEntity->e.shaderRGBA[2] * 0.25f / 255.0f);
	tess.numVertexes++;

	VectorMA(start, spanWidth2, up, tess.xyz[tess.numVertexes]);
	tess.texCoords[tess.numVertexes][0][0] = 0;
	tess.texCoords[tess.numVertexes][0][1] = 1;
	tess.vertexColors[tess.numVertexes][0] = (vec_t)(backEnd.currentEntity->e.shaderRGBA[0] / 255.0f);
	tess.vertexColors[tess.numVertexes][1] = (vec_t)(backEnd.currentEntity->e.shaderRGBA[1] / 255.0f);
	tess.vertexColors[tess.numVertexes][2] = (vec_t)(backEnd.currentEntity->e.shaderRGBA[2] / 255.0f);
	tess.numVertexes++;
...
static void DoRailDiscs(int numSegs, const vec3_t start, const vec3_t dir, const vec3_t right, const vec3_t up)
{
	...
	for (i = 0; i < numSegs; i++)
	{
		...
		for (j = 0; j < 4; j++)
		{
			...
			tess.texCoords[tess.numVertexes][0][0] = (float)(j < 2);
			tess.texCoords[tess.numVertexes][0][1] = (float)(j && j != 3);
			tess.vertexColors[tess.numVertexes][0] = (float)(backEnd.currentEntity->e.shaderRGBA[0] / 255.0f);
			tess.vertexColors[tess.numVertexes][1] = (float)(backEnd.currentEntity->e.shaderRGBA[1] / 255.0f);
			tess.vertexColors[tess.numVertexes][2] = (float)(backEnd.currentEntity->e.shaderRGBA[2] / 255.0f);
			tess.numVertexes++;
...
static void RB_SurfaceLightningBolt(void) {
	...
	for (i = 0; i < 4; i++) {
		...
		DoRailCore(start, end, right, (float)len, 8.0f);
