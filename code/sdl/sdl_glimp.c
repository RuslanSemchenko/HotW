...
void (APIENTRYP qglLockArraysEXT)(int, int);
...
static void GLimp_DetectAvailableModes(void)
{
	...
	numModes = SDL_GetNumVideoDisplays();
	if (numModes < 1) {
	...
	modes = (SDL_DisplayMode*)malloc(sizeof(SDL_DisplayMode) * numModes);
	if (!modes) {
		ri.Error(ERR_FATAL, "Failed to allocate memory for display modes");
	}
...
static qboolean GLimp_StartDriverAndSetMode(int mode, qboolean fullscreen, qboolean noborder)
{
	...
	if (!SDL_WasInit(SDL_INIT_VIDEO))
	{
		...
		Q_strncpyz(driverName, SDL_GetCurrentVideoDriver(), sizeof(driverName));
...
success:
	...
	// Use SDL2 function
	glConfig.deviceSupportsGamma = SDL_SetWindowGammaRamp(sdl_window, NULL, NULL, NULL) >= 0;
...
void GLimp_EndFrame(void)
{
	...
	{
		// Changed to use SDL2 function with window handle
		if (sdl_window) {
	...
	if (r_fullscreen->modified)
	{
		...
		if (sdl_window)
		{
			...
			if (needToToggle)
			{
				// Use new SDL2 function for toggling fullscreen
				if (SDL_SetWindowFullscreen(sdl_window, r_fullscreen->integer ? SDL_WINDOW_FULLSCREEN : 0) < 0)
				{
