...
===============
*/
static const char *IN_TranslateSDLToQ3Key( const SDL_Keysym *keysym,
...
static void IN_GobbleMotionEvents( void )
{
	...
	SDL_PumpEvents( );
	while( SDL_PeepEvents( dummy, 1, SDL_PEEKEVENT, SDL_MOUSEMOTION, SDL_MOUSEMOTION ) ) { }
}
/*
===============
IN_ActivateMouse
...
static void IN_ActivateMouse( void )
{
	...
	if( !mouseActive )
	{
		...
		SDL_ShowCursor( 0 );
#endif
		// In SDL2, SDL_WM_GrabInput is replaced by SDL_SetRelativeMouseMode.
		// Setting relative mode automatically grabs the mouse and hides the cursor.
		// However, the original code had a separate SDL_ShowCursor(0)
		// and SDL_WM_GrabInput(), so we'll set relative mode and also handle
		// the `in_nograb` cvar below with `SDL_SetWindowGrab`.
...
static void IN_DeactivateMouse( void )
{
	...
	if( mouseActive )
	{
		...
		if( SDL_GetWindowFlags(window) & SDL_WINDOW_INPUT_FOCUS )
			SDL_WarpMouseInWindow( window, cls.glconfig.vidWidth / 2, cls.glconfig.vidHeight / 2 );
...
static void IN_InitJoystick( void )
{
	...
	// SDL2 does not use joystick event state.
	...
	for (i = 0; i < total; i++)
	{
		// In SDL2, SDL_JoystickName() now takes an instance ID
	...
	// The new way to get the name is via SDL_JoystickName() with the joystick pointer.
...
static void IN_ProcessEvents( void )
{
	...
	// In SDL2, key repeat is handled internally.
	// The function SDL_EnableKeyRepeat() and related constants are deprecated.
	...
	while( SDL_PollEvent( &e ) )
	{
		switch( e.type )
		{
			...
			// Handle mouse wheel events with the new SDL_MOUSEWHEEL event type
			case SDL_MOUSEWHEEL:
				if (e.wheel.y > 0) {
			...
			// Handle window events for resize and focus
...
void IN_InitKeyLockStates(void)
{
   const unsigned char *keystate = SDL_GetKeyboardState(NULL);
    ...
    // Константы клавиш для блокировок изменились.
    // Мы используем версии SDL_SCANCODE, которые теперь проверяются с помощью SDL_GetKeyboardState.
    keys[K_SCROLLOCK].down = keystate[SDL_SCANCODE_SCROLLLOCK];
    // Удаляем строку с NUMLOCK, так как она не определена в SDL2
    // keys[K_KP_NUMLOCK].down = keystate[SDL_SCANCODE_NUMLOCK]; // Удалите или закомментируйте эту строку
...
void IN_Init( void )
{
	...
	// In SDL2, UNICODE input and key repeat are enabled by default.
	// These functions and related constants are deprecated and removed.

	mouseAvailable = ( in_mouse->value != 0 );
	IN_DeactivateMouse( );

	// In SDL2, app state is handled via SDL_WINDOWEVENT events.
	// This initial check for app state is no longer valid.
	// The event handling logic in IN_ProcessEvents will set these cvars correctly.
