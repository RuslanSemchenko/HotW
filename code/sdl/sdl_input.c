...
===============
*/
static void IN_PrintKey(SDL_Scancode scancode, SDL_Keycode sym, SDL_Keymod mod, keyNum_t key, qboolean down)
{
	if (down)
		Com_Printf("+ ");
	else
	...
	Com_Printf("0x%02x \"%s\"", scancode, SDL_GetKeyName(sym));

	if (mod & SDL_KMOD_LSHIFT)   Com_Printf(" KMOD_LSHIFT");
	if (mod & SDL_KMOD_RSHIFT)   Com_Printf(" KMOD_RSHIFT");
	if (mod & SDL_KMOD_LCTRL)    Com_Printf(" KMOD_LCTRL");
	if (mod & SDL_KMOD_RCTRL)    Com_Printf(" KMOD_RCTRL");
	if (mod & SDL_KMOD_LALT)     Com_Printf(" KMOD_LALT");
	if (mod & SDL_KMOD_RALT)     Com_Printf(" KMOD_RALT");
	if (mod & SDL_KMOD_LGUI)     Com_Printf(" KMOD_LGUI");
	if (mod & SDL_KMOD_RGUI)     Com_Printf(" KMOD_RGUI");
	if (mod & SDL_KMOD_NUM)      Com_Printf(" KMOD_NUM");
	if (mod & SDL_KMOD_CAPS)     Com_Printf(" KMOD_CAPS");
	if (mod & SDL_KMOD_MODE)     Com_Printf(" KMOD_MODE");
...
===============
*/
static const char* IN_TranslateSDLToQ3Key(SDL_Scancode scancode, SDL_Keycode sym, SDL_Keymod mod, keyNum_t* key, qboolean down)
{
	...
	*key = 0;
	...
	else
	{
		...
		case SDLK_KP_9:         *key = K_PGUP;          break;
		case SDLK_PAGEDOWN:     *key = K_PGDN;          break;
		case SDLK_KP_3:         *key = K_PGDN;          break;
		case SDLK_KP_7:         *key = K_HOME;          break;
		case SDLK_HOME:         *key = K_HOME;          break;
		case SDLK_END:          *key = K_END;           break;
		case SDLK_KP_6:         *key = K_KP_RIGHTARROW; break;
		case SDLK_RIGHT:        *key = K_RIGHTARROW;    break;
		case SDLK_KP_2:         *key = K_DOWNARROW;  break;
	...
	if (in_keyboardDebug->integer)
		IN_PrintKey(scancode, sym, mod, *key, down);
...
static void IN_GobbleMotionEvents(void)
{
	...
	SDL_PumpEvents();
	while (SDL_PeepEvents(dummy, 1, SDL_PEEKEVENT, SDL_EVENT_MOUSE_MOTION, SDL_EVENT_MOUSE_MOTION)) {}
}
/*
===============
IN_ActivateMouse
...
static void IN_ActivateMouse(void)
{
	...
	if (!mouseAvailable || !window || !SDL_WasInit(SDL_INIT_VIDEO))
		return;
	...
	if (!mouseActive)
	{
		SDL_HideCursor(); // SDL3 replacement for SDL_ShowCursor(0)
		...
		SDL_HideCursor();
#endif
		// In SDL3, relative mouse mode also hides the cursor and grabs input
	...
	if (!Cvar_VariableIntegerValue("r_fullscreen"))
	{
		if (in_nograb->modified || !mouseActive)
		{
			if (in_nograb->integer)
				SDL_SetWindowMouseGrab(window, false); // SDL3 uses boolean true/false
			else
...
static void IN_DeactivateMouse(void)
{
	...
	if (!Cvar_VariableIntegerValue("r_fullscreen"))
		SDL_ShowCursor(); // SDL3 replacement for SDL_ShowCursor(1)
	...
	if (mouseActive)
	{
		...
		{
			Com_Printf("Warning: Failed to disable relative mouse mode: %s\n", SDL_GetError());
		}

		// Don't warp the mouse unless the cursor is within the window
		if (SDL_GetWindowFlags(window) & SDL_WINDOW_INPUT_FOCUS)
			SDL_WarpMouseInWindow(window, (float)(cls.glconfig.vidWidth / 2), (float)(cls.glconfig.vidHeight / 2));
...
static void IN_InitJoystick(void)
{
	...
	// Решение: переносим буфер в статическую память. 
	// Теперь он занимает 0 байт в стеке.
	static char buf[16384];
	buf[0] = '\0'; // Очищаем строку перед использованием
	...
	if (joysticks && total > 0) {
		// Проверяем индекс, чтобы не выйти за пределы массива joysticks
		int idx = in_joystickNo->integer;
		if (idx >= 0 && idx < total) {
			stick = SDL_OpenJoystick(joysticks[idx]);
		}
	}
...
static void IN_ProcessEvents(void)
{
	...
	while (SDL_PollEvent(&e))
	{
		...
		case SDL_EVENT_KEY_DOWN:
			character = IN_TranslateSDLToQ3Key(e.key.scancode, e.key.key, e.key.mod, &key, qtrue);
			if (key)
		...
		case SDL_EVENT_KEY_UP:
			IN_TranslateSDLToQ3Key(e.key.scancode, e.key.key, e.key.mod, &key, qfalse);
		...
		case SDL_EVENT_TEXT_INPUT:
		{
			...
			{
				Com_QueueEvent(0, SE_CHAR, e.text.text[i], 0, 0, NULL);
				i++;
			}
		}
		break;
		...
		case SDL_EVENT_MOUSE_MOTION:
			if (mouseActive)
				Com_QueueEvent(0, SE_MOUSE, (int)e.motion.xrel, (int)e.motion.yrel, 0, NULL);
			break;
		...
		case SDL_EVENT_QUIT:
			...
			// SDL3 has separate events for window actions, not a single SDL_WINDOWEVENT
		case SDL_EVENT_WINDOW_RESIZED:
		{
			...
			Cvar_Set("r_mode", "-1");
			/* wait until user stops dragging for 1 second */
...
void IN_Frame(void)
{
	...
	else if (window && !(SDL_GetWindowFlags(window) & SDL_WINDOW_INPUT_FOCUS))
	{
...
{
	// В SDL3 GetKeyboardState возвращает указатель на массив bool
