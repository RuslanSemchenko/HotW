...
int	SV_NumForGentity(sharedEntity_t* ent) {
	...
	// FIX C4244: Explicit cast from ptrdiff_t to int
...
intptr_t SV_GameSystemCalls(intptr_t* args) {
	...
	case G_FS_FOPEN_FILE:
		return FS_FOpenFileByMode(VMA(1), VMA(2), (int)args[3]);
	case G_FS_READ:
		FS_Read2(VMA(1), (int)args[2], (int)args[3]);
		return 0;
	case G_FS_WRITE:
		FS_Write(VMA(1), (int)args[2], (int)args[3]);
		return 0;
	case G_FS_FCLOSE_FILE:
		FS_FCloseFile((int)args[1]);
		return 0;
	case G_FS_GETFILELIST:
		return FS_GetFileList(VMA(1), VMA(2), VMA(3), (int)args[4]);
	case G_FS_SEEK:
		return FS_Seek((int)args[1], (int)args[2], (int)args[3]);
	...
	case G_ADJUST_AREA_PORTAL_STATE:
		SV_AdjustAreaPortalState(VMA(1), (int)args[2]);
		return 0;
	...
	case BOTLIB_USER_COMMAND:
		SV_ClientThink(&svs.clients[(int)args[1]], VMA(2));
		return 0;
	...
	case BOTLIB_AAS_PREDICT_ROUTE:
		return botlib_export->aas.AAS_PredictRoute(VMA(1), (int)args[2], VMA(3), (int)args[4], (int)args[5], (int)args[6], (int)args[7], (int)args[8], (int)args[9], (int)args[10], (int)args[11]);

	case BOTLIB_AAS_SWIMMING:
		return botlib_export->aas.AAS_Swimming(VMA(1));
	case BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT:
		return botlib_export->aas.AAS_PredictClientMovement(VMA(1), (int)args[2], VMA(3), (int)args[4], (int)args[5],
			VMA(6), VMA(7), (int)args[8], (int)args[9], VMF(10), (int)args[11], (int)args[12], (int)args[13]);
	...
	case BOTLIB_AI_FIND_MATCH:
		return botlib_export->ai.BotFindMatch(VMA(1), VMA(2), (int)args[3]);
	...
	case BOTLIB_AI_REPLACE_SYNONYMS:
		botlib_export->ai.BotReplaceSynonyms(VMA(1), (int)args[2]);
		return 0;
	...
	case TRAP_MEMSET:
		Com_Memset(VMA(1), (int)args[2], (int)args[3]);
		return 0;

	case TRAP_MEMCPY:
		Com_Memcpy(VMA(1), VMA(2), (int)args[3]);
		return 0;

	case TRAP_STRNCPY:
		strncpy(VMA(1), VMA(2), (int)args[3]);
		return args[1];
