...
static void SV_ClipMoveToEntities(moveclip_t* clip) {
	...
	// =========================================================================
	// ИСПРАВЛЕНИЕ: Добавлена проверка на валидность индекса (>= 0 и < MAX_GENTITIES)
	// Это защищает от краша, если passEntityNum равен -1 (0xffffffff)
	// =========================================================================
	if (clip->passEntityNum != ENTITYNUM_NONE && clip->passEntityNum >= 0 && clip->passEntityNum < MAX_GENTITIES) {
	...
	for (i = 0; i < num; i++) {
		...
		// Дополнительная защита: если указатель пустой (чего быть не должно, но всё же)
		if (!touch) {
			continue;
		}
