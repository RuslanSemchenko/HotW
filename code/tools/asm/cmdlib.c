...
=================
*/
__attribute__((noreturn)) void Error( const char *error, ... )
{
	va_list argptr;
	char	text[1024];
	char	text2[1024];
...
char *ExpandPathAndArchive (const char *path)
{
	...
	if (archive)
	{
		// Check for potential overflow before string creation
		if ( ( strlen( archivedir ) + 1 + strlen( path ) ) >= sizeof( archivename ) ) {
...
int Q_filelength (FILE *f)
{
	...
	if (end < 0)
		return 0; // Ensure non-negative length
	return end;
...
//
	...
	strncat (path, extension, 1024 - strlen(path) - 1);
}
...
void DefaultPath (char *path, const char *basepath)
{
	...
	strncpy(path, basepath, sizeof(path) - 1);
...
//
	...
	Q_strncpyz (dest,src, 1024); // FIXME: This is unsafe
}
