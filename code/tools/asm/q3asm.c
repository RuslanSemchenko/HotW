...
static void WriteMapFile( void ) {
	FILE		*f;
	symbol_t	*s;
	char		imageName[MAX_OS_PATH] = {0};
	int			seg;

	snprintf( imageName, sizeof(imageName), "%s", outputFilename );
	StripExtension( imageName );
	strncat( imageName, ".map", sizeof(imageName) - strlen(imageName) - 1 );
...
static void WriteVmFile( void ) {
	...
	snprintf( imageName, sizeof(imageName), "%s", outputFilename );
	StripExtension( imageName );
	strncat( imageName, ".qvm", sizeof(imageName) - strlen(imageName) - 1 );
...
static void Assemble( void ) {
	...
	for ( i = 0 ; i < numAsmFiles ; i++ ) {
		Q_strncpyz( filename, asmFileNames[ i ], sizeof(filename) );
...
static void ParseOptionFile( const char *filename ) {
	...
	while( ( text_p = COM_Parse( text_p ) ) != 0 ) {
		if ( !strcmp( com_token, "-o" ) ) {
			...
			if (text_p) {
				if ( strstr( com_token, ".." ) ) {
					Error( "Relative pathnames are not allowed for output file." );
				}
				snprintf( outputFilename, sizeof(outputFilename), "%s", com_token );
			}
			continue;
		}
...
//	_chdir( "/quake3/jccode/cgame/lccout" );	// hack for vc profiler
	...
	Q_strncpyz( outputFilename, "q3asm", sizeof(outputFilename) );
	numAsmFiles = 0;	
...
/* Timbo of Tremulous pointed out -o not working; stock ID q3asm folded in the change. Yay. */
			Q_strncpyz( outputFilename, argv[ i+1 ], sizeof(outputFilename) );
			i++;
			continue;
		}
		...
		if ( !strcmp( argv[i], "-f" ) ) {
			...
			snprintf( outputFilename, sizeof(outputFilename), "%s", argv[ i+1 ] );
			i++;
			continue;
		}
