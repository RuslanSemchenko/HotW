...
doinclude(Tokenrow *trp)
{
	...
	if (fname[0]=='/') {
		fd = open(fname, 0);
		strncpy(iname, fname, sizeof(iname)-1); iname[sizeof(iname)-1] = 0;
		...
		if (strlen(fname)+strlen(ip->file)+2 > sizeof(iname))
			continue;
		strncpy(iname, ip->file, sizeof(iname)-1); iname[sizeof(iname)-1] = 0;
		Q_strcat(iname, sizeof(iname), "/");
		Q_strcat(iname, sizeof(iname), fname);
...
genline(void)
{
	...
	int len;
	size_t remaining_size = sizeof(outbuf) - (outp - outbuf);

	ta.t = (uchar*)outp;
	if (cursource->filename[0]!='/' && wd[0]) {
		len = snprintf(outp, remaining_size, "#line %d \"%s/%s\"\n",
			cursource->line, wd, cursource->filename);
	} else {
		len = snprintf(outp, remaining_size, "#line %d \"%s\"\n",
			cursource->line, cursource->filename);
	}
	if (len < 0 || (size_t)len >= remaining_size)
		error(FATAL, "genline buffer overflow");
	ta.len = len;
	outp += len;
	tr.tp = tr.bp;
	puttokens(&tr);
...
setobjname(char *f)
{
	...
	strncpy(objname,f, n+5);
	if(objname[n-2]=='.'){
		strncpy(objname+n-1,"$O: ", 5);
	}else{
		strncpy(objname+n,"$O: ", 5);
	}
}
