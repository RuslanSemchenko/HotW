	...
	for (i = j = 0; cmd[i]; i++) {
		...
		if (s && isdigit(s[1])) {
			...
			if ((b = lists[k-1])) {
				...
				Q_strncpyz(av[j], cmd[i], s - cmd[i] + 1);
				Q_strcat(av[j], strlen(cmd[i]) + strlen(b->str) - 1, b->str);
				Q_strcat(av[j++], strlen(cmd[i]) + strlen(b->str) - 1 + strlen(s+2), s + 2);
				while (b != lists[k-1]) {
					b = b->link;
					assert(j < ac);
					av[j++] = b->str;
