	...
	for (i = 1; i < argc; i++)
		if (strcmp(argv[i], "-T") == 0)
			...
			if (strcmp(argv[i], "-") == 0)
				...
				if (strstr(argv[i], "..")) {
					yyerror("path traversal is not allowed in input filename '%s'\n", argv[i]);
					exit(1);
				}
			...
			if (strcmp(argv[i], "-") == 0)
				...
				if (strstr(argv[i], "..")) {
					yyerror("path traversal is not allowed in output filename '%s'\n", argv[i]);
					exit(1);
				}
