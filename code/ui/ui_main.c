...
// ==================================================================
// ИСПРАВЛЕНИЕ: Используем __declspec(dllexport) вместо Q_EXPORT
// ==================================================================
__declspec(dllexport) intptr_t vmMain(int command, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10, int arg11) {
#ifdef HotW
#ifndef _WIN32
	Send_KeyBindings();
#endif
		...
		while (s && *s && count < len) {
			...
			if (Q_IsColorString(s)) {
				...
				if (style == ITEM_TEXTSTYLE_SHADOWED || style == ITEM_TEXTSTYLE_SHADOWEDMORE) {
					...
					Text_PaintChar(x + (float)ofs, y - yadj + (float)ofs,
						glyph->imageWidth,
						glyph->imageHeight,
						useScale,
						glyph->s,
...
static void UI_SetCapFragLimits(qboolean uiVars) {
	char buffer[64]; // Локальный безопасный буфер
	...
	// Безопасная установка captureLimit
	memset(buffer, 0, sizeof(buffer));
	Com_sprintf(buffer, sizeof(buffer), "%d", cap);
	trap_Cvar_Set(uiVars ? "ui_captureLimit" : "capturelimit", buffer);

	// Безопасная установка fragLimit
	memset(buffer, 0, sizeof(buffer));
	Com_sprintf(buffer, sizeof(buffer), "%d", frag);
	trap_Cvar_Set(uiVars ? "ui_fragLimit" : "fraglimit", buffer);
	...
	/*
	   ВАЖНО: Код выше сбрасывает фраги на 10.
	   Если вы хотите, чтобы эта функция СОХРАНЯЛА то, что вы ввели (999),
	   а не сбрасывала на 10, нужно раскомментировать блок ниже:
	*/
	/*
	if (!uiVars) {
		// Читаем текущие значения из UI перед отправкой в серверные переменные
		score = (int)trap_Cvar_VariableValue("ui_sv_scoreLimit");
		frag  = (int)trap_Cvar_VariableValue("ui_sv_fragLimit");
		duel  = (int)trap_Cvar_VariableValue("ui_sv_duellimit");
		time  = (int)trap_Cvar_VariableValue("ui_sv_timelimit");
	}
	*/

	if (uiVars) {
		// Установка ui_sv_scoreLimit
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", score);
		trap_Cvar_Set("ui_sv_scoreLimit", buffer);

		// Установка ui_sv_fragLimit
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", frag);
		trap_Cvar_Set("ui_sv_fragLimit", buffer);

		// Установка ui_sv_duellimit
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", duel);
		trap_Cvar_Set("ui_sv_duellimit", buffer);

		// Установка ui_sv_timelimit
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", time);
		trap_Cvar_Set("ui_sv_timelimit", buffer);
	}
	else {
		// Установка scorelimit
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", score);
		trap_Cvar_Set("scorelimit", buffer);

		// Установка fraglimit (ИМЕННО ТУТ БЫЛ КРАШ)
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", frag);
		trap_Cvar_Set("fraglimit", buffer);

		// Установка duellimit
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", duel);
		trap_Cvar_Set("duellimit", buffer);

		// Установка timelimit
		memset(buffer, 0, sizeof(buffer));
		Com_sprintf(buffer, sizeof(buffer), "%d", time);
		trap_Cvar_Set("timelimit", buffer);
	}
#endif
}
...
static void UI_DrawNetGameType(rectDef_t* rect, float scale, vec4_t color, int textStyle) {
	// Исправлено условие: индекс должен быть СТРОГО МЕНЬШЕ количества элементов
	// Также добавлена локальная переменная для индекса, чтобы избежать изменения значения cvar в процессе
	int gameTypeIdx = ui_netGameType.integer;
	if (gameTypeIdx < 0 || gameTypeIdx >= uiInfo.numGameTypes) {
		trap_Cvar_Set("ui_netGameType", "0");
		trap_Cvar_Set("ui_actualNetGameType", "0");
		gameTypeIdx = 0; // Сбрасываем локальный индекс для безопасной отрисовки
	}
	// Теперь обращение к массиву безопасно
	if (uiInfo.numGameTypes > 0) {
		Text_Paint(rect->x, rect->y, scale, color, uiInfo.gameTypes[gameTypeIdx].gameType, 0, 0, textStyle);
	}
}

static void UI_DrawJoinGameType(rectDef_t* rect, float scale, vec4_t color, int textStyle) {
	int gameTypeIdx = ui_joinGameType.integer;

	// Исправлено: заменено > на >= для предотвращения выхода за границы
	if (gameTypeIdx < 0 || gameTypeIdx >= uiInfo.numJoinGameTypes) {
		trap_Cvar_Set("ui_joinGameType", "0");
		gameTypeIdx = 0; // Безопасно сбрасываем для текущего кадра отрисовки
	}
	// Проверка на случай, если список типов игр пуст
	if (uiInfo.numJoinGameTypes > 0) {
		Text_Paint(rect->x, rect->y, scale, color, uiInfo.joinGameTypes[gameTypeIdx].gameType, 0, 0, textStyle);
	}
}
...
static void UI_DrawTeamMember(rectDef_t* rect, float scale, vec4_t color, qboolean blue, int num, int textStyle) {
	...
	// 1 - Human
	// 2..NumCharacters - Bot
	int value = (int)trap_Cvar_VariableValue(va(blue ? "ui_blueteam%i" : "ui_redteam%i", num));
	const char* text;
	if (value <= 0) {
		text = "Closed";
	}
...
static void UI_DrawIngameRect(rectDef_t* rect) {
	...
	// top bar
	while (x < (int)rect->w) {
		UI_DrawHandlePic(rect->x + (float)x, rect->y, MENU_WIDTH_WIDTH,
			MENU_WIDTH_HEIGHT, uiInfo.uiDC.Assets.menu_width);

		x += MENU_WIDTH_WIDTH;
	}

	right_border = (int)rect->x + x - MENU_HEIGHT_WIDTH;
	y = MENU_WIDTH_HEIGHT;

	// left/right
	while (y + MENU_WIDTH_HEIGHT < (int)rect->h) {
		UI_DrawHandlePic(rect->x, rect->y + (float)y, MENU_HEIGHT_WIDTH,
			MENU_HEIGHT_HEIGHT, uiInfo.uiDC.Assets.menu_height);

		UI_DrawHandlePic((float)right_border, rect->y + (float)y, MENU_HEIGHT_WIDTH,
			MENU_HEIGHT_HEIGHT, uiInfo.uiDC.Assets.menu_height);

		y += MENU_HEIGHT_HEIGHT;
	}

	bottom_border = (int)rect->y + y;
	x = 0;

	// bottom bar
	while (x < (int)rect->w) {
		UI_DrawHandlePic(rect->x + (float)x, (float)bottom_border, MENU_WIDTH_WIDTH,
			MENU_WIDTH_HEIGHT, uiInfo.uiDC.Assets.menu_width);
	...
	UI_FillRect(rect->x + MENU_HEIGHT_WIDTH, rect->y + MENU_WIDTH_HEIGHT,
		(float)x - 2.0f * MENU_HEIGHT_WIDTH, (float)y - MENU_WIDTH_HEIGHT, color);
}
#endif
...
static const int UI_AIIndexFromName(const char* name) {
	int j;
	for (j = 0; j < uiInfo.aliasCount; j++) {
		if (Q_stricmp(j, uiInfo.aliasList[j].name) == 0) {
...
#ifndef HotW
		LerpColor(color, lowLight, newColor, 0.5f + 0.5f * (float)sin(uiInfo.uiDC.realTime / PULSE_DIVISOR));
#else
		LerpColor(color, lowLight, newColor, 0.5f);
#endif
	...
	if (uiInfo.serverStatus.motdLen) {
		...
		if (uiInfo.uiDC.realTime > uiInfo.serverStatus.motdTime) {
			...
			if (uiInfo.serverStatus.motdPaintX <= rect->x + 2) {
				if (uiInfo.serverStatus.motdOffset < uiInfo.serverStatus.motdLen) {
#ifdef HotW
					uiInfo.serverStatus.motdPaintX += (float)Text_Width(&uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset], scale, 1);
#else
					uiInfo.serverStatus.motdPaintX += (float)Text_Width(&uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset], scale, 1) - 1.0f;
#endif
...
#else
				uiInfo.serverStatus.motdPaintX -= 2.0f;
#endif
				if (uiInfo.serverStatus.motdPaintX2 >= 0) {
#ifdef HotW
					uiInfo.serverStatus.motdPaintX2--;
#else
					uiInfo.serverStatus.motdPaintX2 -= 2.0f;
#endif
				}
			}
		}

		maxX = rect->x + rect->w - 2.0f;
		...
		if (uiInfo.serverStatus.motdPaintX2 >= 0) {
			float maxX2 = rect->x + rect->w - 2.0f;
		...
		if (uiInfo.serverStatus.motdOffset && maxX > 0) {
			...
			if (uiInfo.serverStatus.motdPaintX2 == -1) {
				uiInfo.serverStatus.motdPaintX2 = rect->x + rect->w - 2.0f;
			}
		}
		else {
			uiInfo.serverStatus.motdPaintX2 = -1;
...
#define UI_GL_INFO_TEXTLIMIT 60
	...
	y = (int)rect->y + 45;
	numLines = 0;
	while (y < (int)rect->y + (int)rect->h && *eptr)
	{
		while (*eptr && *eptr == ' ')
			*eptr++ = '\0';
...
#else
		Text_Paint(rect->x + 2, (float)y, scale, color, lines[i++], 0, UI_GL_INFO_TEXTLIMIT / 2 - 1, textStyle);
		if (i < numLines) {
			Text_Paint(rect->x + rect->w / 2, (float)y, scale, color, lines[i++], 0, UI_GL_INFO_TEXTLIMIT / 2, textStyle);
		}
#endif
		y += 10;
		if (y > (int)rect->y + (int)rect->h - 11) {
			break;
		}
	}
...
static qboolean UI_Effects_HandleKey(int flags, float* special, int key) {
	if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER) {
		...
		trap_Cvar_SetValue("color1", (float)uitogamecode[uiInfo.effectsColor]);
		return qtrue;
	}
	return qfalse;
}
...
static void UI_LoadDemos(void) {
	...
	protocolLegacy = (int)trap_Cvar_VariableValue("com_legacyprotocol");
	protocol = (int)trap_Cvar_VariableValue("com_protocol");
...
static void UI_StartSkirmish(qboolean next) {
	...
	trap_Cvar_Set("g_gametype", va("%d", g));
	...
	temp = (int)trap_Cvar_VariableValue("capturelimit");
	trap_Cvar_Set("ui_saveCaptureLimit", va("%i", temp));
	temp = (int)trap_Cvar_VariableValue("fraglimit");
	trap_Cvar_Set("ui_saveFragLimit", va("%i", temp));

	UI_SetCapFragLimits(qfalse);

	temp = (int)trap_Cvar_VariableValue("cg_drawTimer");
	trap_Cvar_Set("ui_drawTimer", va("%i", temp));
	temp = (int)trap_Cvar_VariableValue("g_doWarmup");
	trap_Cvar_Set("ui_doWarmup", va("%i", temp));
	temp = (int)trap_Cvar_VariableValue("g_friendlyFire");
	trap_Cvar_Set("ui_friendlyFire", va("%i", temp));
	temp = (int)trap_Cvar_VariableValue("sv_maxClients");
	trap_Cvar_Set("ui_maxClients", va("%i", temp));
	temp = (int)trap_Cvar_VariableValue("g_warmup");
	trap_Cvar_Set("ui_Warmup", va("%i", temp));
	temp = (int)trap_Cvar_VariableValue("sv_pure");
	...
	if (g == GT_TOURNAMENT) {
		...
		Com_sprintf(buff, sizeof(buff), "wait ; addbot %s %f %s %i \n", uiInfo.mapList[ui_currentMap.integer].opponentName, skill, "", delay);
		trap_Cmd_ExecuteText(EXEC_APPEND, buff);
	}
	else {
...
static void UI_Update(const char* name) {
	...
	if (Q_stricmp(name, "ui_SetName") == 0) {
		...
		case 1: // normal
			...
			trap_Cvar_SetValue("cg_farclip", 1);
			trap_Cvar_SetValue("cg_farclipValue", 1.5f);
			trap_Cvar_SetValue("cg_farclipZoomValue", 4.5f);
			trap_Cvar_SetValue("cg_mapLOD", 2);
#endif
			break;
		case 2: // fast
...
static void UI_RunMenuScript(char** args) {
	...
	if (String_Parse(args, &name)) {
		...
		if (Q_stricmp(name, "StartServer") == 0 ||
			...
			trap_Cvar_SetValue("dedicated", (float)Com_Clamp(0, 2, ui_dedicated.integer));
#else
			...
			trap_Cvar_SetValue("g_gametype", (float)Com_Clamp(0, GT_MAX_GAME_TYPE - 1, uiInfo.gameTypes[ui_netGameType.integer].gtEnum));
#ifndef HotW
			trap_Cvar_Set("g_redTeam", UI_Cvar_VariableString("ui_teamName"));
			trap_Cvar_Set("g_blueTeam", UI_Cvar_VariableString("ui_opponentName"));
#else
			...
			clients = 0;
			for (i = 0; i < PLAYERS_PER_TEAM; i++) {
				int bot = (int)trap_Cvar_VariableValue(va("ui_blueteam%i", i + 1));
				if (bot >= 0) {
					clients++;
				}
				bot = (int)trap_Cvar_VariableValue(va("ui_redteam%i", i + 1));
				if (bot >= 0) {
					clients++;
				}
			}
			...
			if (ui_netSource.integer != UIAS_FAVORITES) {
				...
				Q_strncpyz(serverName, Info_ValueForKey(buff, "hostname"), sizeof(serverName));
			...
			char		info[MAX_INFO_STRING];
			char		ingameServerName[MAX_NAME_LENGTH];
			...
			Q_strncpyz(ingameServerName, Info_ValueForKey(info, "hostname"), MAX_NAME_LENGTH);

			if (strlen(ingameServerName) > 0 && strlen(addr) > 0) {
				res = trap_LAN_AddServer(AS_FAVORITES, ingameServerName, addr);
				if (res == 0) {
					// server already in the list
					Com_Printf("Favorite already in list\n");
				}
			...
			if (strlen(favName) > 0 && strlen(addr) > 0) {
				res = trap_LAN_AddServer(AS_FAVORITES, favName, addr);
				if (res == 0) {
					// server already in the list
					Com_Printf("Favorite already in list\n");
				}
...
static void UI_BuildServerDisplayList(int force) {
	...
	if (len == 0) {
#ifndef HotW
		Q_strncpyz(uiInfo.serverStatus.motd, "Welcome to Team Arena!", sizeof(uiInfo.serverStatus.motd));
#else
		Q_strncpyz(uiInfo.serverStatus.motd, "Welcome to " PRODUCT_NAME, sizeof(uiInfo.serverStatus.motd));
#endif
		len = (int)strlen(uiInfo.serverStatus.motd);
	}
...
static void UI_BuildFindPlayerList(qboolean force) {
	...
	if (!force) {
		...
		// set resend time
		resend = (int)trap_Cvar_VariableValue("ui_serverStatusTimeOut") / 2 - 10;
		if (resend < 50) {
			resend = 50;
		}
	...
	for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
		...
		if (!uiInfo.pendingServerStatus.server[i].valid ||
			uiInfo.pendingServerStatus.server[i].startTime < uiInfo.uiDC.realTime - (int)trap_Cvar_VariableValue("ui_serverStatusTimeOut")) {
			if (uiInfo.pendingServerStatus.server[i].valid) {
				numTimeOuts++;
			}
...
#else
	numdirs = trap_FS_GetFileList("models/wq3_players", "/", dirlist, sizeof(dirlist));
#endif
	dirptr = dirlist;
	for (i = 0; i < numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS; i++, dirptr += dirlen + 1)
	{
...
#ifndef HotW
		numfiles = trap_FS_GetFileList(va("models/players/%s", dirptr), "tga", filelist, sizeof(filelist));
#else
		numfiles = trap_FS_GetFileList(va("models/wq3_players/%s", dirptr), "tga", filelist, sizeof(filelist));
#endif
		fileptr = filelist;
		for (j = 0; j < numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS; j++, fileptr += filelen + 1)
		{
			...
			if (Q_stricmpn(skinname, "icon_", 5) == 0 && !(Q_stricmp(skinname, "icon_blue") == 0 || Q_stricmp(skinname, "icon_red") == 0))
			{
				...
				dirty = 0;
				for (k = 0; k < (size_t)uiInfo.q3HeadCount; k++) {
			...
			{
				if (Q_stricmp(skinname, "icon_default") == 0) {
					Q_strncpyz(uiInfo.q3HeadNames[uiInfo.q3HeadCount], dirptr, sizeof(uiInfo.q3HeadNames[uiInfo.q3HeadCount]));
				}
				else {
...
{
	size_t used;  // длина уже записанной строки

	if (value > 1024 * 1024 * 1024) { // gigs
		Com_sprintf(buf, bufsize, "%d", value / (1024 * 1024 * 1024));
		used = strlen(buf);
		Com_sprintf(buf + used, bufsize - (int)used, ".%02d GB",
			(value % (1024 * 1024 * 1024)) * 100 / (1024 * 1024 * 1024));
	}
	else if (value > 1024 * 1024) { // megs
		Com_sprintf(buf, bufsize, "%d", value / (1024 * 1024));
		used = strlen(buf);
		Com_sprintf(buf + used, bufsize - (int)used, ".%02d MB",
