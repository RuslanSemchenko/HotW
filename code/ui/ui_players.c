...
#else
	if ( weaponNum == WP_PEACEMAKER || weaponNum == WP_KNIFE || weaponNum == WP_GATLING ) {
#endif
...
static void UI_RunLerpFrame(playerInfo_t* ci, lerpFrame_t* lf, int newAnimation) {
	...
	// Проверяем, сменилась ли анимация
	...
	// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Проверка на NULL
	if (!anim) {
	...
	// Если время текущего кадра прошло, переходим к следующему
	if (dp_realtime >= lf->frameTime) {
		lf->oldFrame = lf->frame;
		lf->oldFrameTime = lf->frameTime;

		if (dp_realtime < lf->animationTime) {
			lf->frameTime = lf->animationTime;		// начальный lerp
		}
		else {
		...
		if (f >= anim->numFrames) {
			...
			if (anim->loopFrames) {
				...
				f = anim->numFrames - 1;
				// Анимация закончилась, позволяем немедленный переход
				lf->frameTime = dp_realtime;
			}
		}
	...
	// Защита от десинхронизации времени (например, после паузы или лага)
	...
	// Расчет значения интерполяции (lerp)
