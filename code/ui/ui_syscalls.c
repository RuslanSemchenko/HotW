...
=========================================================================
ui_syscalls.c
ÈÑÏÐÀÂËÅÍÍÛÉ: Óñòðàíåíû âàðíèíãè C4244 (intptr_t -> int/qhandle_t)
=========================================================================
*/
...
int trap_Milliseconds(void) {
	return (int)syscall(UI_MILLISECONDS);
}
...
float trap_Cvar_VariableValue(const char* var_name) {
	floatint_t fi;
	fi.i = (int)syscall(UI_CVAR_VARIABLEVALUE, var_name);
	return fi.f;
}
...
int trap_FS_FOpenFile(const char* qpath, fileHandle_t* f, fsMode_t mode) {
	return (int)syscall(UI_FS_FOPENFILE, qpath, f, mode);
}
...
int trap_FS_GetFileList(const char* path, const char* extension, char* listbuf, int bufsize) {
	return (int)syscall(UI_FS_GETFILELIST, path, extension, listbuf, bufsize);
}

int trap_FS_Seek(fileHandle_t f, long offset, int origin) {
	return (int)syscall(UI_FS_SEEK, f, offset, origin);
}

qhandle_t trap_R_RegisterModel(const char* name) {
	return (qhandle_t)syscall(UI_R_REGISTERMODEL, name);
}

qhandle_t trap_R_RegisterSkin(const char* name) {
	return (qhandle_t)syscall(UI_R_REGISTERSKIN, name);
}
...
qhandle_t trap_R_RegisterShaderNoMip(const char* name) {
	return (qhandle_t)syscall(UI_R_REGISTERSHADERNOMIP, name);
}
...
int trap_CM_LerpTag(orientation_t* tag, clipHandle_t mod, int startFrame, int endFrame, float frac, const char* tagName) {
	return (int)syscall(UI_CM_LERPTAG, tag, mod, startFrame, endFrame, PASSFLOAT(frac), tagName);
}
...
sfxHandle_t trap_S_RegisterSound(const char* sample, qboolean compressed) {
	return (sfxHandle_t)syscall(UI_S_REGISTERSOUND, sample, compressed);
}
...
int trap_Key_GetCatcher(void) {
	return (int)syscall(UI_KEY_GETCATCHER);
}
...
int trap_GetConfigString(int index, char* buff, int buffsize) {
	return (int)syscall(UI_GETCONFIGSTRING, index, buff, buffsize);
}

int	trap_LAN_GetServerCount(int source) {
	return (int)syscall(UI_LAN_GETSERVERCOUNT, source);
}
...
int trap_LAN_GetServerPing(int source, int n) {
	return (int)syscall(UI_LAN_GETSERVERPING, source, n);
}

int trap_LAN_GetPingQueueCount(void) {
	return (int)syscall(UI_LAN_GETPINGQUEUECOUNT);
}

int trap_LAN_ServerStatus(const char* serverAddress, char* serverStatus, int maxLen) {
	return (int)syscall(UI_LAN_SERVERSTATUS, serverAddress, serverStatus, maxLen);
}
...
int trap_LAN_ServerIsVisible(int source, int n) {
	return (int)syscall(UI_LAN_SERVERISVISIBLE, source, n);
}
...
int trap_LAN_AddServer(int source, const char* name, const char* addr) {
	return (int)syscall(UI_LAN_ADDSERVER, source, name, addr);
}
...
int trap_LAN_CompareServers(int source, int sortKey, int sortDir, int s1, int s2) {
	return (int)syscall(UI_LAN_COMPARESERVERS, source, sortKey, sortDir, s1, s2);
}

int trap_MemoryRemaining(void) {
	return (int)syscall(UI_MEMORY_REMAINING);
}
...
int trap_PC_AddGlobalDefine(char* define) {
	return (int)syscall(UI_PC_ADD_GLOBAL_DEFINE, define);
}

int trap_PC_LoadSource(const char* filename) {
	return (int)syscall(UI_PC_LOAD_SOURCE, filename);
}

int trap_PC_FreeSource(int handle) {
	return (int)syscall(UI_PC_FREE_SOURCE, handle);
}

int trap_PC_ReadToken(int handle, pc_token_t* pc_token) {
	return (int)syscall(UI_PC_READ_TOKEN, handle, pc_token);
}

int trap_PC_SourceFileAndLine(int handle, char* filename, int* line) {
	return (int)syscall(UI_PC_SOURCE_FILE_AND_LINE, handle, filename, line);
}
...
int trap_RealTime(qtime_t* qtime) {
	return (int)syscall(UI_REAL_TIME, qtime);
}

int trap_CIN_PlayCinematic(const char* arg0, int xpos, int ypos, int width, int height, int bits) {
	return (int)syscall(UI_CIN_PLAYCINEMATIC, arg0, xpos, ypos, width, height, bits);
}
